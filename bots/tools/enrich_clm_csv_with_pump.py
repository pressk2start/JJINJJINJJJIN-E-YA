...
출력:
  ...
  콘솔에 히스토그램/버킷성과/판정
"""
import sys, csv, json, time, argparse, datetime as dt
...
PNL_CANDS    = ["pnl","ret","return","ret_pct","pnl_pct","profit","수익","손익","result_pct","pct"]
...
def main():
    ...
    ap.add_argument("--tz-offset", type=float, default=0.0,
                    help="CSV timestamp가 KST 등 로컬시각이면 UTC와의 시차(시간). KST면 9")
    a = ap.parse_args()
    ...
    for i, r in enumerate(rows):
        try:
            ems = parse_ts(r[ts_col]) - off_ms   # 로컬->UTC 보정
