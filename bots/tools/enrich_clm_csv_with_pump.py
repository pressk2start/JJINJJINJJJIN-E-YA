...
⚠ CLM CSV 실제 스키마 대응 (bugfix):
  - `time` = 청산시각(KST HH:MM:SS), 진입시각 아님. (momentum_scanner_clm.py L343)
  - `hold_sec` = 진입~청산 초.
  - `pump_1m` = 봇이 진입 순간 이미 정확히 저장 (L1072, 신규 세션만).
  처리 순서:
    (1) 신규 세션 (pump_1m 컬럼 존재): 그 값 그대로 사용 (API 재조회 0건)
    (2) 구세션 (pump_1m 없음, hold_sec 있음):
        진입시각 = time - hold_sec 로 복원 후 API 조회
    (3) hold_sec 도 없음: time을 진입시각으로 가정 (fallback)
...
def main():
    ...
    # 🔧 CLM CSV 실제 스키마 대응:
    #   - `time` = 청산시각 (KST HH:MM:SS, momentum_scanner_clm.py line 343)
    #   - `hold_sec` = 진입~청산 초 (같은 파일 line 340)
    #   - `pump_1m` = 봇이 진입 순간 이미 정확히 저장 (line 1072, 신규 세션만)
    #
    # 처리 우선순위:
    #   (1) row에 pump_1m 컬럼이 있고 값이 있으면 → 그대로 사용 (API 호출 0)
    #   (2) 없으면 → resolve_ms(time) - hold_sec 로 진입시각 복원 후 API 조회
    has_pump_col = "pump_1m" in header
    has_hold_col = "hold_sec" in header
    if has_pump_col:
        print("신규 세션 감지: CSV에 pump_1m 컬럼 있음 → 그 값 재사용 (API 재조회 없음)")
    if not has_pump_col and not has_hold_col:
        print("⚠ hold_sec 없음 & pump_1m 없음 → time을 진입시각으로 가정 (구세션 미상)")
    elif not has_pump_col and has_hold_col:
        print("구세션 감지: CSV에 pump_1m 없음, hold_sec 있음 → time-hold_sec로 진입시각 복원 후 API 조회")

    out = []
    t0 = time.time(); fails = 0; reused = 0; recomputed = 0
    ...
    for i, r in enumerate(rows):
        ...
        try:
            ...
            if pump is None:
                ...
                if has_hold_col:
                    try:
                        hold_ms = int(float(r.get("hold_sec", 0)) * 1000)
                    except (ValueError, TypeError):
                        hold_ms = 0
                ems_entry = ems_close - hold_ms
                mkt = norm_market(r[mkt_col])
                pump = prior_minute_pump(mkt, ems_entry)
                if pump is not None:
                    recomputed += 1
        ...
        out.append(r)
        if (i+1) % 25 == 0:
            print(f"  {i+1}/{len(rows)} ... {time.time()-t0:.0f}s  (재사용{reused}, 재조회{recomputed})")
        if pump is None or has_pump_col is False:
            time.sleep(0.05)

    outpath = a.csv.rsplit(".", 1)[0] + "_enriched.csv"
    with open(outpath, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=list(out[0].keys())); w.writeheader(); w.writerows(out)
    print(f"\n저장: {outpath}  (재사용 {reused}건 / 재조회 {recomputed}건 / 실패 {fails}건)")
