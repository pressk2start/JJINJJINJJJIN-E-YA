...
tick_bp = tick / price × 10000. 스켈핑이 노리는 건 1~2틱이므로 이 값이 전부다.
  ...
  · tick_bp 가 크면 (DOGE 81bp) **최소 스프레드 자체가 비용을 지배**한다 → 요구 알파가 급등
  · tick_bp 가 작은 것 자체는 **불리하지 않다**. BTC(0.1bp)는 오히려 스프레드 비용이 최소다.
    BTC 가 스켈핑에 안 맞을 수 있는 이유는 틱이 작아서가 아니라 **단기 변동성 대비
    수수료 부담(fee burden)** 때문이다. "왕복비용까지 200틱 필요"는 틱을 단위로 센
    착시이고 실제 단위는 bp 다. (이전 버전의 tick_bp 하한 게이트는 이 착시에서 나왔다 — 폐기)
...
이다. DOGE 라면 10 + 81 = 91bp 로, 전 종목 공통 가정(21.7bp)의 4배다.

⚠ **이름 그대로 floor 다. 기대 체결비용이 아니다.** 실제 비용은
    fee + spread + 진입 book walk + 청산 book walk + market impact + 지연/역행
  이고 이 함수는 앞의 둘만 본다. WS 가 쌓이면 종목·국면별로
  cost_floor(m,t) / cost_p50(m,regime) / cost_p90(m,regime) 로 가야 한다.

⚠ **높은 tick_bp 는 "거래 불가"가 아니라 "요구 알파가 매우 높다"는 뜻이다.**
  DOGE 처럼 91bp 가 들어도, 가령 P(+2% within 30s | signal) 이 충분히 높은 신호가
  존재한다면 거래는 성립한다. "구조적 불가"라고 부르려면
    최소 체결비용 > 해당 horizon 에서 **검증된** 조건부 엣지의 상한
  이 함께 성립해야 한다. 현재는 그 상한을 모른다.
  따라서 판정은 "현재 연구 유니버스에서 제외하는 것이 합리적"까지다.

⚠ **한 종목에 tick 이 하나라는 가정도 근사다.** tick 은 가격 구간에 따라 바뀌므로
  30단 호가창이 구간 경계를 걸치면 한 스냅샷 안에 두 종류의 간격이 공존한다.
  실측: KRW-DOGE 124원 호가창의 매수호가 간격 = {0.1, 1.0} (100원 경계를 걸침).
