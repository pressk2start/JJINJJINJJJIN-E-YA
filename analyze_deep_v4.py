...
"""
딥 분석 스크립트 v4 (업그레이드)
- 미래 데이터 누수 방지 (to_time=진입시각+1초, candle_time<=target 필터)
- 1분봉만 호출 → 5분봉은 1분봉으로 리샘플 (정렬/일관성 ↑, API 호출 ↓)
- RSI/MACD/Stoch/ADX/ATR 등 지표를 "정식" 구현(라인/시그널/히스토그램, K/D, ADX 등)
- 판별력: diff/std 대신 AUC(=Mann-Whitney U 기반) + 중앙값 차이
- 임계치: 목표 성공통과율 유지하며 실패통과 최소화(방향 자동)
"""
...
def _parse_upbit_1m_candles(raw: List[Dict[str, Any]]) -> List[Candle]:
    candles: List[Candle] = []
    for c in raw:
    ...
    candles.sort(key=lambda x: x.dt_kst)  # chronological
    return candles


# =========================
# Indicators (proper-ish)
# =========================
...
def auc_from_ranks(success: Sequence[float], fail: Sequence[float]) -> Optional[float]:
    """
    AUC = P(score_success > score_fail) + 0.5 * P(equal)
    No scipy. O(n^2) is fine for small dataset.
    """
...
def find_optimal_threshold(
    ...
    """
    direction: ">=" or "<="
    returns (threshold, success_pass_rate, fail_pass_rate)
    """
...
def find_entry_index(candles: Sequence[Candle], target_dt: datetime, max_gap_sec: int = 60) -> Optional[int]:
    """
    Prefer candle exactly at target_dt (Upbit candle_date_time_kst is candle start time).
    Else choose the latest candle with dt <= target_dt and gap <= max_gap_sec.
    """
    exact = [i for i, c in enumerate(candles) if c.dt_kst == target_dt]
    if exact:
        return exact[-1]
...
def analyze_deep_v4(client: UpbitClient, ticker: str, date_str: str, time_str: str) -> Optional[Dict[str, Any]]:
    ...
    candles_1m_all = _parse_upbit_1m_candles(raw_1m)
...
def main() -> None:
    print("=" * 80)
    print("🔬 딥 분석 v4 - 미래누수 제거 + 5분봉 리샘플 + AUC 판별력")
    print("=" * 80)
    ...
    for ticker, date_str, time_str, is_success in CASES:
        ...
        r = analyze_deep_v4(client, ticker, date_str, time_str)
        if r is None:
            print("✗")
            continue
    ...
    # (key, label, s_avg, s_med, f_avg, f_med, auc, mad_pct)
    ...
    for key, label in metrics:
        ...
        # robust dispersion proxy: MAD/|median| (%), median near 0 보호
