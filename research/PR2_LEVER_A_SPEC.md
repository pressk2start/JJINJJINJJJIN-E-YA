...
## 라벨링 규율 — 필터검증 base-rate 노이즈 (advisor 코드 감사 확정 2026-08-01)

**핵심 발견**: 필터검증 줄의 모든 fail 군 wr 이 **~46% 근처 공통 base-rate** 몰림.
```
climax_body_fail: 45·45·45·48·46·47·45·47·46%
vr_fail: 45%  pbr_wick_fail: 48%
climax_vr3_fail: 46%  climax_a2_vr5cap_fail: 47%
```
→ **base-rate ± 3%p (43~49%) 는 노이즈** · **필터 특성 신호 아님**

### A2 등급 조정 (2회 정정)

**1차 (advisor 방향성 일치)**: "climax_vr3_fail 28건 wr46% +0.17% = A2 방향 방증"
**2차 (advisor 코드 감사 정정)**: 46% 는 base-rate · **방증 근거로도 약함**

**A2 최종 위치**:
- 후향 검증 (리서치 코호트 409, 5단계): 강함
- 라이브 paired shadow (CLM_A_x_A2_bp30 n=1): **미착수 · 판정 대기**
