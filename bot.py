...
# EarlyCut shadow — 60s 시점 죽은 거래 조기 절단 (51차 분류 분석: A/C 만족군 PnL-0.64~-0.69%, wr 0%)
def _make_clm_ec(mode, mfe_thr, dd_thr=None):
    ep = {
        "strategy": "TRAIL",
        "sl_pct": 0.020,
        "activation_pct": 1.0,
        ...
        "early_cut_60s": {"mode": mode, "mfe_thr": mfe_thr},
    }
    if dd_thr is not None:
        ep["early_cut_60s"]["dd_thr"] = dd_thr
    return ep

_V0_EXIT_PARAMS_CLM_EC_A = _make_clm_ec("A", 0.001, 0.002)
_V0_EXIT_PARAMS_CLM_EC_C = _make_clm_ec("C", 0.0015)
...
def _shadow_sim_exit(vp, cur_price):
    ...
    # 1.8) Early Cut — 60s 시점 mfe_60s/dd_peak_60s/pnl_60s 기반 죽은 거래 조기 절단
    # 분류 데이터: A(mfe60<0.10% AND dd60>0.20%) 만족 55건 PnL-0.69% wr 0%
    #            C(mfe60<0.15% AND pnl60<0)    만족 65건 PnL-0.64% wr 0%
