...
def run_analysis(all_results, tick_all, ob_all):
    ...
    try:
        ...
        for tf_name, tf_factors in tf_configs:
            ...
            if tf_name == "1m":
                # RSI 구간별 상세
                for lo, hi, label in [(0,30,"과매도"),(30,45,"저역"),(45,55,"중립"),(55,70,"고역"),(70,100,"과매수")]:
                    sub = [r for r in all_results if r.get("rsi_1m") is not None and lo <= r["rsi_1m"] < hi]
                    if len(sub) >= 10:
                        w = sum(r["win"] for r in sub)/len(sub)*100
                        p = safe_mean([r["pnl"] for r in sub])
                        line = f"  RSI1m {label}[{lo}-{hi}): n={len(sub)} WR={w:.1f}% PnL={p:+.3f}%"
                        out_lines.append(line); sub_msg += line + "\n"

                # EMA 정배열 vs 역배열
                aligned = [r for r in all_results if r.get("ema_aligned") == 1]
                not_aligned = [r for r in all_results if r.get("ema_aligned") == 0]
                if len(aligned) >= 10 and len(not_aligned) >= 10:
                    wa = sum(r["win"] for r in aligned)/len(aligned)*100
                    wn = sum(r["win"] for r in not_aligned)/len(not_aligned)*100
                    line = f"  EMA정배열: WR={wa:.1f}%(n={len(aligned)}) vs 비정배열: WR={wn:.1f}%(n={len(not_aligned)})"
                    out_lines.append(line); sub_msg += line + "\n"

                # BB포지션 + 거래량 콤보
                for bb_lo, bb_hi in [(0,20),(20,40),(40,60),(60,80),(80,100)]:
                    high_vol = [r for r in all_results if r.get("bb_pos") is not None and bb_lo<=r["bb_pos"]<bb_hi and r.get("vol_surge_5",0)>=2]
                    if len(high_vol) >= 10:
                        w = sum(r["win"] for r in high_vol)/len(high_vol)*100
                        line = f"  BB[{bb_lo}-{bb_hi})+VS5≥2: n={len(high_vol)} WR={w:.1f}%"
                        out_lines.append(line); sub_msg += line + "\n"

            elif tf_name == "5m":
                # 5분봉 RSI + 거래량 서지
                for lo, hi in [(0,40),(40,55),(55,70),(70,100)]:
                    sub = [r for r in all_results if r.get("rsi_5m") is not None and lo <= r["rsi_5m"] < hi]
                    if len(sub) >= 10:
                        w = sum(r["win"] for r in sub)/len(sub)*100
                        p = safe_mean([r["pnl"] for r in sub])
                        line = f"  RSI5m[{lo}-{hi}): n={len(sub)} WR={w:.1f}% PnL={p:+.3f}%"
                        out_lines.append(line); sub_msg += line + "\n"

                # 5m ATR 구간
                for lo, hi in [(0,0.3),(0.3,0.5),(0.5,0.8),(0.8,999)]:
                    sub = [r for r in all_results if r.get("atr_5m_pct") is not None and lo <= r["atr_5m_pct"] < hi]
                    if len(sub) >= 10:
                        w = sum(r["win"] for r in sub)/len(sub)*100
                        line = f"  ATR5m[{lo}-{hi}): n={len(sub)} WR={w:.1f}%"
                        out_lines.append(line); sub_msg += line + "\n"

            elif tf_name == "15m":
                # 15분봉 RSI 구간
                for lo, hi in [(0,40),(40,55),(55,70),(70,100)]:
                    sub = [r for r in all_results if r.get("rsi_15m") is not None and lo <= r["rsi_15m"] < hi]
                    if len(sub) >= 10:
                        w = sum(r["win"] for r in sub)/len(sub)*100
                        line = f"  RSI15m[{lo}-{hi}): n={len(sub)} WR={w:.1f}%"
                        out_lines.append(line); sub_msg += line + "\n"

            elif tf_name == "1h":
                # 1시간봉 추세 구간별 상세
                for lo, hi, label in [(-999,-0.3,"강하락"),(-0.3,0,"약하락"),(0,0.1,"횡보"),(0.1,0.3,"약상승"),(0.3,999,"강상승")]:
                    sub = [r for r in all_results if r.get("trend_1h") is not None and lo <= r["trend_1h"] < hi]
                    if len(sub) >= 10:
                        w = sum(r["win"] for r in sub)/len(sub)*100
                        p = safe_mean([r["pnl"] for r in sub])
                        line = f"  1H추세 {label}[{lo},{hi}): n={len(sub)} WR={w:.1f}% PnL={p:+.3f}%"
                        out_lines.append(line); sub_msg += line + "\n"

                # 1H RSI 구간
                for lo, hi in [(0,40),(40,55),(55,70),(70,100)]:
                    sub = [r for r in all_results if r.get("rsi_1h") is not None and lo <= r["rsi_1h"] < hi]
                    if len(sub) >= 10:
                        w = sum(r["win"] for r in sub)/len(sub)*100
                        line = f"  RSI1h[{lo}-{hi}): n={len(sub)} WR={w:.1f}%"
                        out_lines.append(line); sub_msg += line + "\n"

            msg += sub_msg
            if len(msg) > 3500:
                tg(msg[:3900]); msg = f"📊 [TF별 독립분석 계속]\n"
    ...
    try:
        ...
        def eval_cond(r, cond):
            """조건 문자열 평가 (안전)"""
            if cond is None: return True
            parts = cond.replace(">=","≥").replace("<=","≤").replace(">","≥").replace("<","≤")
            # 파싱: "rsi_1m≥55" or "rsi_1m≤45"
            for op_str, op_sym in [("≥", ">="), ("≤", "<=")]:
                if op_str in parts:
                    key, val = parts.split(op_str, 1)
                    key = key.strip()
                    try: val = float(val.strip())
                    except: return True
                    rv = r.get(key)
                    if rv is None: return False
                    if op_sym == ">=": return rv >= val
                    if op_sym == "<=": return rv <= val
            return True

        # 조건 파싱 수정: 원본 문자열 직접 파싱
        def eval_simple(r, cond_str):
            if cond_str is None: return True
            # "rsi_1m<40" → key=rsi_1m, op=<, val=40
            for op in [">=","<=",">","<"]:
                if op in cond_str:
        ...
        for c1c, c5c, c15c, c1hc, label in rsi_combos:
            sub = [r for r in all_results
                   if eval_simple(r, c1c) and eval_simple(r, c5c) and eval_simple(r, c15c) and eval_simple(r, c1hc)]
            if len(sub) >= 10:
                w = sum(r["win"] for r in sub)/len(sub)*100
                p = safe_mean([r["pnl"] for r in sub])
                star = " ★" if w >= wr+5 else (" ✗" if w <= wr-5 else "")
        ...
        if len(msg) > 3500:
            tg(msg[:3900]); msg = "🔄 [크로스TF 계속]\n"

        # ATR 다중TF 비교 (1m vs 5m vs 15m 변동성 관계)
        msg += "\n--- ATR 다중TF 비교 ---\n"
        out_lines.append("\n--- ATR 다중TF 비교 ---")
        # 1m ATR > 5m ATR = 단기 변동성 확대 (모멘텀?)
        hi_1m_atr = [r for r in all_results if r.get("atr_pct",0) > 0 and r.get("atr_5m_pct",0) > 0 and r["atr_pct"] > r["atr_5m_pct"]]
        lo_1m_atr = [r for r in all_results if r.get("atr_pct",0) > 0 and r.get("atr_5m_pct",0) > 0 and r["atr_pct"] <= r["atr_5m_pct"]]
        if len(hi_1m_atr) >= 10 and len(lo_1m_atr) >= 10:
            wh = sum(r["win"] for r in hi_1m_atr)/len(hi_1m_atr)*100
            wl = sum(r["win"] for r in lo_1m_atr)/len(lo_1m_atr)*100
            line = f"  1mATR>5mATR(단기폭발): n={len(hi_1m_atr)} WR={wh:.1f}% vs 반대: n={len(lo_1m_atr)} WR={wl:.1f}%"
            out_lines.append(line); msg += line + "\n"

        # RSI 발산(다이버전스): 1m RSI 상승 + 5m RSI 하락 등
        div_up = [r for r in all_results if r.get("rsi_1m",50)>55 and r.get("rsi_5m",50)<45]  # 1m강+5m약
        div_dn = [r for r in all_results if r.get("rsi_1m",50)<45 and r.get("rsi_5m",50)>55]  # 1m약+5m강
        conv = [r for r in all_results if abs(r.get("rsi_1m",50)-r.get("rsi_5m",50))<10]      # 수렴
        for sub, label in [(div_up,"RSI발산(1m↑5m↓)"),(div_dn,"RSI발산(1m↓5m↑)"),(conv,"RSI수렴(1m≈5m)")]:
            if len(sub) >= 10:
                w = sum(r["win"] for r in sub)/len(sub)*100
                p = safe_mean([r["pnl"] for r in sub])
                line = f"  {label}: n={len(sub)} WR={w:.1f}% PnL={p:+.3f}%"
                out_lines.append(line); msg += line + "\n"

        # 최적 진입 조건 탐색: 모든 TF 정렬시
        all_align_bull = [r for r in all_results
                          if r.get("rsi_1m",50)>55 and r.get("rsi_5m",50)>55
                          and r.get("rsi_15m",50)>55 and r.get("rsi_1h",50)>55
                          and r.get("trend_1h",0)>0]
        all_align_bear = [r for r in all_results
                          if r.get("rsi_1m",50)<45 and r.get("rsi_5m",50)<45
                          and r.get("rsi_15m",50)<45 and r.get("rsi_1h",50)<45]
        for sub, label in [(all_align_bull,"전TF정배열(Bull)"),(all_align_bear,"전TF역배열(Bear)")]:
            if len(sub) >= 5:
                w = sum(r["win"] for r in sub)/len(sub)*100
                p = safe_mean([r["pnl"] for r in sub])
                line = f"  ⭐ {label}: n={len(sub)} WR={w:.1f}% PnL={p:+.3f}%"
        ...
        msg += "\n--- 일봉추세 × 1m지표 ---\n"
        out_lines.append("\n--- 일봉추세 × 1m지표 ---")
        for dlo, dhi, dlabel in [(-999,-0.5,"일봉하락"),(-0.5,0.5,"일봉횡보"),(0.5,999,"일봉상승")]:
            sub = [r for r in all_results if r.get("daily_trend") is not None and dlo <= r["daily_trend"] < dhi]
            if len(sub) >= 10:
                w = sum(r["win"] for r in sub)/len(sub)*100
                p = safe_mean([r["pnl"] for r in sub])
                star = " ★" if w >= wr+5 else (" ✗" if w <= wr-5 else "")
                line = f"  {dlabel}: n={len(sub)} WR={w:.1f}% PnL={p:+.3f}%{star}"
                out_lines.append(line); msg += line + "\n"
                # 세부: 일봉추세 + EMA break
                eb1 = [r for r in sub if r.get("ema_break")==1]
                eb0 = [r for r in sub if r.get("ema_break")==0]
                if len(eb1)>=5 and len(eb0)>=5:
                    w1 = sum(r["win"] for r in eb1)/len(eb1)*100
                    w0 = sum(r["win"] for r in eb0)/len(eb0)*100
                    line = f"    +EMA돌파: WR={w1:.1f}%(n={len(eb1)}) vs +EMA하방: WR={w0:.1f}%(n={len(eb0)})"
                    out_lines.append(line); msg += line + "\n"
