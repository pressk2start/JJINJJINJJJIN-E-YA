# -*- coding: utf-8 -*-
"""
업비트 24시간 종합 캔들 분석 v3.5 (v3 + NO-LEAK + Detailed Stats 통합)
=========================================================================
- 거래량 상위 50종목
- 1분/5분/15분/1시간/일봉 + 틱(3000건+) + 호가창(스냅샷)
...
[중요 통합 변경]
1) ✅ 틱 누수 방지:
   - 기존 v3: 과거 idx에서도 "현재 기준 tick3000"을 그대로 사용 → 미래 틱 섞일 수 있음
   - 통합본: tick을 timestamp 오름차순 정렬 후,
            simulate()에서 idx 시점(ms) 기준 lookback 윈도우만 slice하여 tick feature 계산

2) ✅ 통계 더 디테일:
   - v3 섹션 전부 유지 + v4의 time-split(앞/중/뒤) + decile(10분위) 추가

3) ✅ orderbook:
   - 업비트는 과거 호가 스냅샷을 못 가져옴 → idx feature로 쓰면 누수/왜곡 위험
   - 따라서 "현재 스냅샷 요약"만 coin 단위로 저장/리포트(기존 ORDERBOOK SUMMARY 유지)
"""
...
# ===== NO-LEAK tick window 설정 =====
TICK_LOOKBACK_SEC = 90     # idx 시점 기준 과거 90초 tick만 사용
SIM_HORIZON_BARS = 15      # 1m 15봉(=15분) 미래로 시뮬
...
def get_ticks_paginated(market, target=3000):
    """
    tick 대량수집 (cursor paging)
    - 반환은 timestamp(ms) 기준 오름차순 정렬(과거→현재)
    - simulate()에서 idx 시점 기준으로 slice해서 누수 방지
    """
    all_ticks = []
    cursor = None
...
def obv_trend(candles, period=10):
    ...
    if len(obvs) < period:
        return 0.0
    return (obvs[-1] - obvs[-period]) / max(abs(obvs[-period]), 1.0)
...
def adx_approx(candles, period=14):
    ...
    dx = abs(pdi-mdi) / max(pdi+mdi, 1e-6) * 100
    return round(dx, 1)
...
# ================================================================
# 5) 틱 분석 (v3 deep + v4 window)
# ================================================================
def analyze_ticks_deep(trades):
    """
    v3의 deep 요약(코인 최신 스냅샷/참고용)
    - trades 정렬 방향이 섞여도 동작하도록 내부에서 timestamp 기준 정렬
    """
    if not trades or len(trades) < 50:
        return {}
    ts = [t for t in trades if t.get("timestamp")]
    if ts:
        ts.sort(key=lambda x: x["timestamp"], reverse=True)  # 최신 우선(기존 로직 유지 느낌)
        trades = ts
    ...
    if len(timestamps) >= 2:
        time_span = (max(timestamps) - min(timestamps)) / 1000
        tps = len(trades) / max(time_span, 1)
    else:
        tps = 0
    ...
    # v4에서 추가한 cv(체결 간격 변동성)도 deep snapshot에 포함
    cv = 0.0
    try:
        if len(timestamps) >= 6:
            gaps = [(timestamps[i]-timestamps[i+1])/1000 for i in range(len(timestamps)-1) if timestamps[i] and timestamps[i+1]]
            gaps = [g for g in gaps if g >= 0]
            if len(gaps) >= 5:
                mu = statistics.mean(gaps)
                sd = statistics.stdev(gaps) if len(gaps) >= 2 else 0
                cv = sd / mu if mu > 1e-6 else 0
    except:
        cv = 0.0
...
def analyze_ticks_window(trades_sorted_asc):
    """
    v4의 NO-LEAK window feature
    trades_sorted_asc: timestamp 오름차순(과거→현재) slice
    """
    if not trades_sorted_asc or len(trades_sorted_asc) < 10:
        return {
            "n": len(trades_sorted_asc) if trades_sorted_asc else 0,
            "buy_ratio": 0.5,
            "buy_30": 0.5,
            "buy_100": 0.5,
            "big_buy_ratio": 0.5,
    ...
    total = len(trades_sorted_asc)
    buys = sum(1 for t in trades_sorted_asc if t.get("ask_bid") == "BID")

    prices = [t.get("trade_price", 0) for t in trades_sorted_asc if t.get("trade_price", 0) > 0]
    if not prices:
        prices = [1.0]
    mn = statistics.mean(prices)
    std = statistics.stdev(prices) if len(prices) >= 2 else 0.0
    pstd = (std/mn*100) if mn > 0 else 0.0

    timestamps = [t.get("timestamp") for t in trades_sorted_asc if t.get("timestamp")]
    if len(timestamps) >= 2:
        span = (max(timestamps) - min(timestamps)) / 1000.0
        tps = total / max(span, 1e-6)
    else:
        tps = 0.0
    ...
    try:
        if len(timestamps) >= 6:
            ...
            for i in range(1, len(timestamps)):
                g = (timestamps[i] - timestamps[i-1]) / 1000.0
                if g >= 0:
                    gaps.append(g)
            if len(gaps) >= 5:
                mu = statistics.mean(gaps)
                sd = statistics.stdev(gaps) if len(gaps) >= 2 else 0.0
                cv = (sd/mu) if mu > 1e-6 else 0.0
    except:
        cv = 0.0

    recent30 = trades_sorted_asc[-30:] if total >= 30 else trades_sorted_asc
    buy_30 = sum(1 for t in recent30 if t.get("ask_bid")=="BID") / max(len(recent30), 1)
    recent100 = trades_sorted_asc[-100:] if total >= 100 else trades_sorted_asc
    buy_100 = sum(1 for t in recent100 if t.get("ask_bid")=="BID") / max(len(recent100), 1)

    vals = [ (t.get("trade_price",0) * t.get("trade_volume",0)) for t in trades_sorted_asc ]
    vals_sorted = sorted([v for v in vals if v > 0])
    if vals_sorted:
        thr = vals_sorted[int(len(vals_sorted)*0.95)]
        big_idx = [i for i, v in enumerate(vals) if v >= thr and v > 0]
        big_count = len(big_idx)
        big_buy = sum(1 for i in big_idx if trades_sorted_asc[i].get("ask_bid")=="BID")
        big_buy_ratio = big_buy / max(big_count, 1)
    else:
        big_buy_ratio = 0.5
    ...
    if len(prices) >= 25:
        ...
        if older and statistics.mean(older) > 0:
            momentum = (statistics.mean(recent)/statistics.mean(older) - 1) * 100.0
    ...
    return {
        "n": total,
        "buy_ratio": round(buys/total, 3),
        "buy_30": round(buy_30, 3),
        "buy_100": round(buy_100, 3),
        "big_buy_ratio": round(big_buy_ratio, 3),
        "tps": round(tps, 2),
        "pstd": round(pstd, 4),
        "cv": round(cv, 4),
...
def volume_profile(candles, n_bins=10):
    ...
    for p, v in zip(prices, vols):
        idx = min(int((p - mn_p) / max(bin_size, 1e-9)), n_bins - 1)
        bins[idx] += v
...
def kst_to_ts_ms(kst_str: str) -> int:
    try:
...
def nearest_idx(candles, t_kst_str):
    # candles는 시간 오름차순 가정
    lo, hi = 0, len(candles) - 1
    ans = 0
    while lo <= hi:
        mid = (lo + hi) // 2
        if candles[mid]["candle_date_time_kst"] <= t_kst_str:
            ans = mid
            lo = mid + 1
        else:
            hi = mid - 1
...
def slice_ticks_before(ticks_sorted_asc, until_ts_ms: int, lookback_sec: int):
    if not ticks_sorted_asc or until_ts_ms <= 0:
        return []
    lo_ts = until_ts_ms - int(lookback_sec * 1000)

    # bisect for speed
    import bisect
    ts_list = [t["timestamp"] for t in ticks_sorted_asc]
    left = bisect.bisect_left(ts_list, lo_ts)
    right = bisect.bisect_right(ts_list, until_ts_ms)
    return ticks_sorted_asc[left:right]

# ================================================================
# 8) 시뮬레이션 (NO-LEAK tick integrated)
# ================================================================
def simulate(c1, c5, c15, c60, daily, market, btc_c5, ticks_sorted_asc):
    results = []
    if len(c1) < 40:
        return results
    ...
    for idx in range(30, len(c1) - (SIM_HORIZON_BARS + 1), 3):
        c = c1[idx]
        prev = c1[idx-1]
        close = c["trade_price"]
        op = c["opening_price"]
        ...
        ema5_prev = ema(closes[:-1], 5) if len(closes) >= 6 else None
        ...
        # ===== 다중 타임프레임 =====
        i5 = nearest_idx(c5, tkst)
        rsi5=50; atr5p=0; vs5m=1
        if i5 >= 15 and len(c5) > i5:
            ...
            vs5m = vol_surge_at(c5, i5, 3)

        i15 = nearest_idx(c15, tkst)
        rsi15=50; atr15p=0
        ...
        # ===== NO-LEAK tick window (idx 시점 이전만) =====
        tick_slice = slice_ticks_before(ticks_sorted_asc, t_ms, TICK_LOOKBACK_SEC)
        tickw = analyze_ticks_window(tick_slice)
        ...
        for fwd in range(1, min(SIM_HORIZON_BARS+1, len(c1)-idx)):
            fc = c1[idx+fwd]
            ...
            if not armed and g >= cp:
                armed = True
                ts = max(ep*(1+FEE), fcl*(1-td))
            if armed:
                ts = max(ts, fh*(1-td))
                if fl <= ts:
                    exit_p = ts; exit_r = "TRAIL"; exit_b = fwd; break

        if exit_p is None:
            exit_p = c1[min(idx+SIM_HORIZON_BARS, len(c1)-1)]["trade_price"]
            exit_r = "TIMEOUT"
            exit_b = SIM_HORIZON_BARS
        ...
        results.append({
            ...
            "vol_surge_5": round(vs,2),
            "vol_surge_10": round(vs_10,2),
            ...
            "bb_width": bb_w,
            "bb_pos": round(bb_pos,1) if bb_pos else None,
            ...
            # tick (NO-LEAK window features)
            "tick_n": tickw.get("n", 0),
            "tick_buy_ratio": tickw.get("buy_ratio", 0.5),
            "tick_buy_30": tickw.get("buy_30", 0.5),
            "tick_buy_100": tickw.get("buy_100", 0.5),
            "tick_big_buy": tickw.get("big_buy_ratio", 0.5),
            "tick_tps": tickw.get("tps", 0.0),
            "tick_pstd": tickw.get("pstd", 0.0),
            "tick_cv": tickw.get("cv", 0.0),
            "tick_momentum": tickw.get("momentum", 0.0),
            "tick_total_krw": tickw.get("total_krw", 0.0),
...
# ================================================================
# v4 추가: time-split + decile 리포트(텔레그램 디테일 강화)
# ================================================================
def _format_wr_pnl(rs):
    if not rs:
        return "n=0 WR=0.0% PnL=+0.000%"
    w = sum(r["win"] for r in rs)/len(rs)*100
    p = safe_mean([r["pnl"] for r in rs])
    return f"n={len(rs)} WR={w:5.1f}% PnL={p:+.3f}%"

def time_split_report(all_results):
    if len(all_results) < 300:
        return None
    rs = sorted(all_results, key=lambda x: x.get("ts_ms", 0))
    n = len(rs)
    a = rs[:n//3]
    b = rs[n//3:2*n//3]
    c = rs[2*n//3:]
    return [
        "⏱️ [TIME-SPLIT] (앞/중/뒤 3분할)",
        f"  앞  : {_format_wr_pnl(a)}",
        f"  중  : {_format_wr_pnl(b)}",
        f"  뒤  : {_format_wr_pnl(c)}",
    ]

def decile_report(all_results, field, wr_base):
    vs = [r for r in all_results if r.get(field) is not None]
    if len(vs) < 120:
        return None
    vs.sort(key=lambda x: x[field])
    n = len(vs)
    lines = []
    wrs = []
    for d in range(10):
        ...
        if not chunk:
            continue
        wr = sum(r["win"] for r in chunk)/len(chunk)*100
        pnl = safe_mean([r["pnl"] for r in chunk])
        wrs.append(wr)
        lo = chunk[0][field]
        hi = chunk[-1][field]
        star = " ★" if wr >= wr_base + 5 else (" ✗" if wr <= wr_base - 5 else "")
        lines.append(f"  D{d+1:02d} [{lo:.3f}..{hi:.3f}] n={len(chunk):4d} WR={wr:5.1f}% PnL={pnl:+.3f}%{star}")
    trend = ""
    if len(wrs) >= 6:
        low = statistics.mean(wrs[:3])
        high = statistics.mean(wrs[-3:])
        if high - low > 5:
            trend = " (WR↑ with value)"
        elif low - high > 5:
            trend = " (WR↓ with value)"
    header = f"🔟 [DECILE] {field}{trend}"
    return header, lines

# ================================================================
# 9) 분석 (v3 섹션 유지 + v4 time-split/decile 추가)
# ================================================================
def run_analysis(all_results, tick_all_snapshot, ob_all_snapshot):
    out_lines = []
    try:
    ...
    # time-split(추가)
    try:
        tsr = time_split_report(all_results)
        if tsr:
            out_lines.extend([""] + tsr)
            tg("\n".join(tsr)[:3900])
    except Exception as e:
        tg(f"❌ time-split 에러: {e}")

    # 팩터 중요도 (v3 유지 + tick 확장항목 추가)
    imp = []
    try:
        factors = [
            ...
            # tick (NO-LEAK)
            "tick_buy_ratio","tick_buy_30","tick_buy_100","tick_big_buy","tick_tps","tick_pstd","tick_cv","tick_momentum","tick_total_krw","tick_n"
        ]
        for f in factors:
    ...
    # decile(추가): 중요도 상위 10개 변수에 대해 10분위 상세
    try:
        top_for_decile = [x["factor"] for x in imp[:10]] if imp else []
        for f in top_for_decile:
            rep = decile_report(all_results, f, wr)
            if not rep:
                continue
            header, lines = rep
            out_lines.append("\n" + header)
    ...
    # 팩터 구간 상세 (v3 유지 + tick bins 확장)
    try:
        bins_map = {
            "body_pct":[-999,0,0.3,0.5,1,2,999],
            "vol_surge_5":[-999,0.5,1,2,5,10,999],
            "rsi_1m":[-999,30,50,60,70,80,999],
            "rsi_5m":[-999,30,50,60,70,80,999],
            "rsi_1h":[-999,30,50,60,70,80,999],
            "trend_1h":[-999,-0.3,0,0.1,0.3,0.5,999],
            "ema_gap":[-999,-0.5,0,0.3,0.5,1,2,999],
            "vwap_gap":[-999,-1,-0.3,0,0.3,1,2,999],
            "green_streak":[0,1,2,3,4,5,999],
            "atr_pct":[0,0.2,0.3,0.5,0.8,1,999],
            "bb_pos":[-999,20,40,60,80,100,999],
            "stoch_k":[-999,20,40,60,80,100,999],
            "adx":[-999,15,25,40,60,999],
            "daily_trend":[-999,-2,-0.5,0,0.5,2,999],

            # tick (NO-LEAK)
            "tick_buy_ratio":[0,0.3,0.4,0.5,0.6,0.7,1],
            "tick_buy_30":[0,0.3,0.4,0.5,0.6,0.7,1],
            "tick_buy_100":[0,0.3,0.4,0.5,0.6,0.7,1],
            "tick_big_buy":[0,0.3,0.5,0.6,0.7,0.8,1],
            "tick_tps":[0,1,3,5,8,12,20,999],
            "tick_pstd":[0,0.02,0.05,0.10,0.20,0.40,999],
            "tick_cv":[0,0.3,0.6,1.0,1.5,2.5,999],
        ...
        for fn, bins in bins_map.items():
            ...
            if len(msg) > 3500:
                tg(msg[:3900])
                msg = "📈 [FACTOR DETAIL 계속]\n"
        if msg.strip():
            tg(msg[:3900])
    except Exception as e:
        tg(f"❌ 팩터 구간 에러: {e}")

    # 콤보 (v3 유지 + tick combo 추가)
    try:
        combos = [
            ...
            # tick combos
            ("tick_buy_ratio",0.6,"vol_surge_5",2),
            ("tick_big_buy",0.6,"rsi_1m",55),
            ("tick_tps",5,"tick_buy_ratio",0.55),
            ("tick_cv",1.2,"tick_pstd",0.10),

            ("daily_trend",0.5,"vol_surge_5",2),
            ("btc_corr",0.5,"trend_1h",0.1),
        ]
        msg = "🔗 [COMBOS]\n"
        out_lines.append("\n[COMBOS]")
    ...
    try:
        ...
        if mfes:
            line = "[MFE] " + " ".join(f"P{p}:+{mfes[int(len(mfes)*p/100)]:.3f}%" for p in [25,50,75,90] if int(len(mfes)*p/100) < len(mfes))
            out_lines.append(line); msg += line + "\n"
        if maes:
            line = "[MAE] " + " ".join(f"P{p}:{maes[int(len(maes)*p/100)]:.3f}%" for p in [10,25,50,75] if int(len(maes)*p/100) < len(maes))
            out_lines.append(line); msg += line + "\n"
    ...
    # 틱 요약 (스냅샷) - v3 유지 but 내용 확장 가능
    try:
        if tick_all_snapshot:
            msg = "📊 [TICK SUMMARY]\n"
            out_lines.append("\n[TICK SUMMARY]")
            for m, td in sorted(tick_all_snapshot.items(), key=lambda x:-x[1].get("buy_ratio",0)):
                line = f"  {m.split('-')[1]:8s}: 매수비={td.get('buy_ratio',0):.0%} 대량매수비={td.get('big_buy_ratio',0):.0%} 초당={td.get('tps',0):.1f} CV={td.get('cv',0):.2f} pstd={td.get('pstd',0):.3f}% n={td.get('n',0)}"
                out_lines.append(line)
                msg += line + "\n"
            tg(msg[:3900])
    except Exception as e:
    ...
    try:
        if ob_all_snapshot:
            ...
            out_lines.append("\n[ORDERBOOK SUMMARY]")
            for m, od in sorted(ob_all_snapshot.items(), key=lambda x:-x[1].get("imbalance",0)):
    ...
    try:
        ...
        tf_configs = [
            ("1m", ["rsi_1m","atr_pct","ema_gap","ema5_slope","bb_width","bb_pos","stoch_k","obv_trend","adx","macd_hist","vwap_gap","body_pct","vol_surge_5","vol_vs_ma20","mom5","mom10","price_chg","uwr","lwr","green_streak",
                    "tick_buy_ratio","tick_buy_30","tick_big_buy","tick_tps","tick_cv","tick_pstd"]),
            ("5m", ["rsi_5m","atr_5m_pct","vol_surge_5m"]),
            ("15m", ["rsi_15m","atr_15m_pct"]),
            ("1h", ["rsi_1h","trend_1h"]),
        ]

        for tf_name, tf_factors in tf_configs:
            sub_msg = f"\n--- {tf_name} 봉 독립분석 ---\n"
            out_lines.append(f"\n--- {tf_name} 봉 독립분석 ---")
            ...
            for f in tf_factors:
                ...
                w4 = sum(r["win"] for r in q4)/len(q4)*100
                d = "↑" if w4 > w1 else "↓"
                line = f"  {f:17s}: Q1 WR={w1:.1f}% Q4 WR={w4:.1f}% diff={w4-w1:+.1f}% {d}"
                out_lines.append(line); sub_msg += line + "\n"

            msg += sub_msg
            if len(msg) > 3500:
                tg(msg[:3900]); msg = "📊 [TF별 독립분석 계속]\n"
    ...
    try:
        ...
        rsi_combos = [
            ("rsi_1m<40", "rsi_5m<40", None, None, "1m+5m 과매도"),
            ("rsi_1m>60", "rsi_5m>60", None, None, "1m+5m 과매수"),
            ("rsi_1m<50", "rsi_5m>55", None, None, "1m저+5m고 (5m선행)"),
            ("rsi_1m>55", "rsi_5m<50", None, None, "1m고+5m저 (1m선행)"),
            ("rsi_1m<50", None, "rsi_15m<50", None, "1m+15m 약세"),
            ("rsi_1m>55", None, "rsi_15m>55", None, "1m+15m 강세"),
            ("rsi_1m>55", "rsi_5m>55", "rsi_15m>55", None, "1m+5m+15m 삼중강세"),
            ("rsi_1m<50", "rsi_5m<50", "rsi_15m<50", None, "1m+5m+15m 삼중약세"),
            (None, None, None, "rsi_1h>60", "1H RSI강세"),
            (None, None, None, "rsi_1h<40", "1H RSI약세"),
            ("rsi_1m>55", None, None, "rsi_1h>55", "1m+1h 동시강세"),
            ("rsi_1m>55", None, None, "rsi_1h<45", "1m강+1h약 (괴리)"),
            ("rsi_1m<45", None, None, "rsi_1h>55", "1m약+1h강 (눌림?)"),
        ]

        def eval_simple(r, cond_str):
            if cond_str is None:
                return True
            for op in [">=","<=",">","<"]:
                if op in cond_str:
                    key, val = cond_str.split(op, 1)
                    key = key.strip()
                    try:
                        val = float(val.strip())
                    except:
        ...
        for c1c, c5c, c15c, c1hc, label in rsi_combos:
            sub = [r for r in all_results if eval_simple(r, c1c) and eval_simple(r, c5c) and eval_simple(r, c15c) and eval_simple(r, c1hc)]
            if len(sub) >= 10:
                w = sum(r["win"] for r in sub)/len(sub)*100
                p = safe_mean([r["pnl"] for r in sub])
                star = " ★" if w >= wr+5 else (" ✗" if w <= wr-5 else "")
                line = f"  {label:22s}: n={len(sub):5d} WR={w:5.1f}% PnL={p:+.3f}%{star}"
                out_lines.append(line); msg += line + "\n"

        msg += "\n--- 1H추세 × 1m RSI 교차 ---\n"
        out_lines.append("\n--- 1H추세 × 1m RSI 교차 ---")
        trend_bins = [(-999,-0.1,"하락"),(-0.1,0.1,"횡보"),(0.1,999,"상승")]
        rsi_bins = [(0,45,"저RSI"),(45,60,"중RSI"),(60,100,"고RSI")]
        for tlo, thi, tlabel in trend_bins:
            for rlo, rhi, rlabel in rsi_bins:
                sub = [r for r in all_results
                       if r.get("trend_1h") is not None and tlo <= r["trend_1h"] < thi
                       and r.get("rsi_1m") is not None and rlo <= r["rsi_1m"] < rhi]
                if len(sub) >= 10:
                    w = sum(r["win"] for r in sub)/len(sub)*100
                    p = safe_mean([r["pnl"] for r in sub])
                    star = " ★" if w >= wr+5 else (" ✗" if w <= wr-5 else "")
                    line = f"  1H{tlabel}+{rlabel}: n={len(sub):5d} WR={w:5.1f}% PnL={p:+.3f}%{star}"
                    out_lines.append(line); msg += line + "\n"

        msg += "\n--- VWAP × EMA × 1H추세 교차 ---\n"
        out_lines.append("\n--- VWAP × EMA × 1H추세 교차 ---")
        for vwap_cond, vlabel in [("below","VWAP아래"),("above","VWAP위")]:
            for ema_cond, elabel in [("below","EMA아래"),("above","EMA위")]:
                for trend_cond, trlabel in [("down","1H하락"),("up","1H상승")]:
                    sub = [r for r in all_results
                           if r.get("vwap_gap") is not None and r.get("ema_gap") is not None and r.get("trend_1h") is not None
                           and ((vwap_cond=="below" and r["vwap_gap"]<0) or (vwap_cond=="above" and r["vwap_gap"]>=0))
                           and ((ema_cond=="below" and r["ema_gap"]<0) or (ema_cond=="above" and r["ema_gap"]>=0))
                           and ((trend_cond=="down" and r["trend_1h"]<0) or (trend_cond=="up" and r["trend_1h"]>=0))]
                    if len(sub) >= 10:
                        w = sum(r["win"] for r in sub)/len(sub)*100
                        p = safe_mean([r["pnl"] for r in sub])
                        star = " ★" if w >= wr+5 else (" ✗" if w <= wr-5 else "")
                        line = f"  {vlabel}+{elabel}+{trlabel}: n={len(sub):5d} WR={w:5.1f}% PnL={p:+.3f}%{star}"
                        out_lines.append(line); msg += line + "\n"
...
def main():
    t0 = time.time()
    tg("🔍 [캔들분석v3.5] 시작\n"
       f"TOP{TOP_N} × (1m/5m/15m/1h/일봉 + tick3000 + orderbook snapshot)\n"
       "✅ tick feature는 idx 시점 이전 데이터만 사용 (NO-LEAK)\n"
       "📨 각 분석 섹션 텔레그램 즉시 전송")
    ...
    all_results = []
    tick_all_snapshot, ob_all_snapshot = {}, {}
    import gc
    ...
    for i, m in enumerate(markets):
        ...
        try:
            ...
            ticks_sorted = get_ticks_paginated(m, 3000); time.sleep(0.1)
            ...
            # tick snapshot(참고용): v3 deep + v4 확장 포함
            ts = analyze_ticks_deep(ticks_sorted)
            if ts:
                tick_all_snapshot[m] = ts

            # orderbook snapshot(참고용)
            obs = analyze_orderbook(ob)
            if obs:
                ob_all_snapshot[m] = obs

            # NO-LEAK simulate
            res = simulate(c1, c5, c15, c60, daily, m, btc_c5, ticks_sorted)
            all_results.extend(res)
            print(f"  시뮬: {len(res)}건 WR={sum(r['win'] for r in res)/max(len(res),1)*100:.1f}%")

            del c1, c5, c15, c60, daily, ticks_sorted, ob, res
            gc.collect()
    ...
    out_lines, imp = run_analysis(all_results, tick_all_snapshot, ob_all_snapshot)
    ...
    try:
        ...
        summary = {
            ...
            "importance": imp,
            "tick_snapshot": tick_all_snapshot,
            "orderbook_snapshot": ob_all_snapshot,
            "wr": round(sum(r["win"] for r in all_results)/len(all_results)*100, 1),
            "pnl": round(safe_mean([r["pnl"] for r in all_results]), 3),
            "note": {
                "no_leak_tick_window_sec": TICK_LOOKBACK_SEC,
                "sim_horizon_bars": SIM_HORIZON_BARS,
                "orderbook_used_in_features": False
            }
        }
    ...
    tg(f"✅ [캔들분석v3.5] 전체 완료! | {len(all_results)}건 | {elapsed:.1f}분")
