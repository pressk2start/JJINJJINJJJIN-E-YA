...
HARD_STOP_DD = 0.015  # 🔧 476건분석: 3.2→1.5% (SL 0.7%×2.1, 비상청산)
...
DYN_SL_MIN = 0.007   # 🔧 476건분석: MFE중앙값0.30%, SL2%는 과대 → 0.7% (BB<50+13-20시 PF3.80)
DYN_SL_MAX = 0.015   # 🔧 476건분석: SL_MIN×2.1 (고변동 코인 여유)

# 🔧 통합 체크포인트: 트레일링/얇은수익/Plateau 발동 기준
# 🔧 구조개선: SL 연동 — 체크포인트 = SL × 1.5 (의미있는 수익에서만 트레일 무장)
#   기존 0.30%에서 무장 → 진입가+0.06%에 트레일스톱 → 한 틱에 트립 문제 해결
PROFIT_CHECKPOINT_BASE = 0.005  # 🔧 476건분석: 0.25%→0.5% (trail 0.8%에 맞춤, 무장 후 바로 트립 방지)
...
MFE_RR_MULTIPLIERS = {
    "🔥점화": 1.0,              # SL 0.7%×1.0=0.7%
    "강돌파 (EMA↑+고점↑)": 1.43, # SL 0.7%×1.43=1.0%
    "EMA↑": 1.0,                 # SL 0.7%×1.0=0.7%
    "고점↑": 1.0,                # SL 0.7%×1.0=0.7%
    "거래량↑": 1.0,              # SL 0.7%×1.0=0.7%
    "기본": 1.0,                 # SL 0.7%×1.0=0.7%
}
...
# 트레일링 손절 설정
# 🔧 v6 백테스트 최적화: Trail 0.5% 고정
# 근거: 84개 트레일 설정 중 Trail 0.5%가 최적 (PnL +74.23%, PF 1.84)
#       0.15%는 정상 눌림에서 조기 트립, 0.8% 이상은 수익 감소
TRAIL_ATR_MULT = 1.0  # ATR 기반 여유폭
TRAIL_DISTANCE_MIN_BASE = 0.008  # 🔧 476건분석: 0.5%→0.8% (SL 좁아진 만큼 trail 넓혀 정상눌림 트립 방지)

def get_trail_distance_min():
    """🔧 v6: Trail 0.5% 고정 (시간대 차등 제거 — 13-17시만 거래하므로 불필요)"""
...
def _apply_exit_profile():
    ...
    if prof == "gentle":
        WARMUP_SEC = 7
        HARD_STOP_DD = 0.018   # SL 0.7%×2.5
        EXIT_DEBOUNCE_SEC = 8
        EXIT_DEBOUNCE_N = 3
        TRAIL_ATR_MULT = 1.2
        TRAIL_DISTANCE_MIN_BASE = 0.010  # 🔧 476건: trail 넓게 (gentle)
        ...
        WARMUP_SEC = 3
        HARD_STOP_DD = 0.012   # SL 0.7%×1.7
        EXIT_DEBOUNCE_SEC = 6
        EXIT_DEBOUNCE_N = 3
        TRAIL_ATR_MULT = 0.90
        TRAIL_DISTANCE_MIN_BASE = 0.006  # 🔧 476건: trail 타이트 (strict)
        ...
        WARMUP_SEC = 5
        HARD_STOP_DD = 0.015   # 🔧 476건: SL 0.7%×2.1
        EXIT_DEBOUNCE_SEC = 10
        EXIT_DEBOUNCE_N = 3
        TRAIL_ATR_MULT = 1.0
        TRAIL_DISTANCE_MIN_BASE = 0.008  # 🔧 476건: trail 0.8%
        SPIKE_RECOVERY_WINDOW = 3
        SPIKE_RECOVERY_MIN_BUY = 0.58
        CTX_EXIT_THRESHOLD = 3
...
# 손절/모니터링
STOP_LOSS_PCT = 0.007  # 🔧 DYN_SL_MIN 0.7% 연동 (폴백용)
...
POSTCHECK_MAX_DD = 0.015  # 🔧 476건: HARD_STOP_DD 1.5% 연동
...
def auto_learn_exit_params():
    ...
    try:
        ...
        if "mae_pct" in df.columns:
            ...
            if len(loss_mae) >= 5:
                ...
                if avg_loss_mae >= current_sl_pct * 0.80:
                    ...
                    new_sl = max(0.005, min(0.010, round(new_sl, 4)))
        ...
        # =====================================================
        # 2) DYN_SL_MAX = DYN_SL_MIN × 2.1 연동 (바운드: 1.0~2.0%)
        # =====================================================
        new_sl_max = round(DYN_SL_MIN * 2.1, 4)
        ...
        # =====================================================
        # 3) HARD_STOP_DD = DYN_SL_MIN × 2.1 연동 (바운드: 1.0~2.5%)
        # =====================================================
        new_hard = round(DYN_SL_MIN * 2.1, 4)
        new_hard = max(0.010, min(0.025, new_hard))
...
def detect_leader_stock(m, obc, c1, tight_mode=False):
    ...
    if not _ign_candidate:
        try:
            ...
            # 🔧 v6 RSI 필터: RSI >= 60 → 진입 차단 (과매수 구간 회피)
            # 근거: RSI60-70 WR=9.2% (최저), RSI<30 WR=22.5% (최고)
            if _chart_rsi >= 60:
                return None
            ...
            if _bb_position >= 50:
                print(f"[BB_CUT] {m} BB{_bb_position:.0f}≥50 → 볼린저상단 차단")
                return None

            # 🔧 476건분석: VR≥5 차단 (VR5+ WR0%, avg-0.42% → 꼭대기 진입)
            if _chart_volratio >= 5.0:
                print(f"[VR_HIGH_CUT] {m} VR{_chart_volratio:.1f}≥5.0 → 과열거래량 차단")
                return None

            # 🔧 476건분석: RSI<50 → half (약세장, WR29%)
            # RSI≥60 이미 차단, VR≥5 이미 차단 → 나머지 override 불필요 (정리됨)
            if _chart_rsi < 50:
                print(f"[WEAK_RSI] {m} RSI{_chart_rsi:.0f}<50 → 약세 half")
    ...
    # 🔧 v6: 오후 half 패널티 제거 (13-17시가 유일한 거래 시간이므로 불필요)
...
def dynamic_stop_loss(entry_price, c1, signal_type=None, current_price=None, trade_type=None, market=None):
    ...
    # 🔧 476건분석: 전시간 SL 0.7% 통일 (13-20시만 거래)
