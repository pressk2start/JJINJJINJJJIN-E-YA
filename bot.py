...
# ========================================
GATE_TREND_1H_MIN = -0.3         # 🔧 캔들분석: 1H 추세 -0.3% 이하 → 진입 차단 (WR=20.7%✗)
GATE_TREND_1H_HALF = 0.0         # 🔧 캔들분석: 1H 추세 0% 이하 → half 강제 (WR=24.6%)
...
def detect_leader_stock(m, obc, c1, tight_mode=False):
    ...
    # 🔧 캔들분석: 시간대 half (23,250건 HOURLY 기반)
    # 08시 WR=20.5%, 11시 WR=23.7%, 13시 WR=23.3%, 17시 WR=22.3%, 21시 WR=23.1%
    # → 08시 추가, 17시까지 확대 (기존 13~16시 → 08시+11시+13~17시+21시)
    if not _ign_candidate and _hour_kst in (8, 11, 13, 14, 15, 16, 17, 21):
