...
PROFIT_CHECKPOINT_BASE = 0.0060  # 🔧 대규모백테스트: 0.25→0.60% (236K캔들 30코인×4TF: CP0.60%+Trail0.15%가 전구간 최적, PnL -436→-159 개선)
...
TRAIL_DISTANCE_MIN_BASE = 0.0040  # 🔧 R:R개선: 0.15→0.40% (알트 정상 눌림 0.3~0.7% 중간값, 0.15%는 노이즈에 트립)
...
def context_exit_score(m, ticks, ob_depth_krw, entry_price, last_price, c1):
    ...
    # 🔧 R:R개선: FLOW_DROP + NO_UPTICK 복합 조건만 인정 (개별 +1점씩→복합 +2점)
    # 기존: 각각 score+1 → 단일 조건에 민감하게 청산 유도
    # 수정: 둘 다 동시 충족 시에만 score+2 (진짜 추세 역전만 감지)
    ...
    if _flow_drop and _no_uptick:
        score += 2
        reasons.append("FLOW_DROP+NO_UPTICK")
