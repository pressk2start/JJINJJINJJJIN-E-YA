...
PROFIT_CHECKPOINT_BASE = 0.0045  # 🔧 R:R개선: 0.35→0.45% (MFE 50%=0.604%, 수수료 0.2% 차감 후 실질 이익 확보 후 Trail 무장)
...
TRAIL_DISTANCE_MIN_BASE = 0.0030  # 🔧 R:R개선: 0.20→0.30% (MAE 25%=0.284%, 정상 눌림 범위 커버)

def get_trail_distance_min():
    """🔧 R:R개선: 실제 데이터 기반 트레일 거리
    MAE 25%=0.284% → 정상 눌림 범위를 커버해야 트레일이 안 끊김
    야간(0-7시) 0.25% (변동성 낮음), 주간 0.30% (SL 1.2% × 0.25)
    """
    _h = now_kst().hour
    if 0 <= _h < 7:
        return 0.0025  # 야간 0.25% (변동성 낮음)
    dyn_sl = DYN_SL_MIN
    return max(TRAIL_DISTANCE_MIN_BASE, dyn_sl * 0.25)  # SL 1.2% × 0.25 = 0.30%
