...
# 🔧 v3.3 백테스트: SL 1.0% (TRAIL_SL1.0/A0.3/T0.2 최적)
DYN_SL_MIN = 0.010   # 🔧 v3.3: SL 1.0% (20봉돌파/거래량3배 최적)
DYN_SL_MAX = 0.015   # 🔧 v3.3: SL_MAX 1.5% (고변동 여유)

# 🔧 v3.3 백테스트: Activation 0.3% (TRAIL_SL1.0/A0.3/T0.2 최적)
PROFIT_CHECKPOINT_BASE = 0.003  # 🔧 v3.3: Arm at +0.3% 수익 시 트레일 무장
...
# 🔧 v6 백테스트 최적화: TP 3.0% 통일 (SL 2.0% × 1.5 = 3.0%)
...
# 트레일링 손절 설정
# 🔧 v3.3 백테스트: Trail 0.2% (TRAIL_SL1.0/A0.3/T0.2 최적)
TRAIL_ATR_MULT = 1.0  # ATR 기반 여유폭
TRAIL_DISTANCE_MIN_BASE = 0.002  # 🔧 v3.3: Trail 0.2% (고점 대비 0.2% 하락 시 청산)
...
def _apply_exit_profile():
    ...
    if prof == "gentle":
        ...
        WARMUP_SEC = 5
        HARD_STOP_DD = 0.015   # 🔧 v3.3: SL 1.0%×1.5
        EXIT_DEBOUNCE_SEC = 10
        EXIT_DEBOUNCE_N = 3
        TRAIL_ATR_MULT = 1.0
        TRAIL_DISTANCE_MIN_BASE = 0.002  # 🔧 v3.3: trail 0.2%
        SPIKE_RECOVERY_WINDOW = 3
        SPIKE_RECOVERY_MIN_BUY = 0.58
        CTX_EXIT_THRESHOLD = 3
...
RETEST_MODE_ENABLED = False          # 🔧 v3.3: 리테스트 비활성화 (백테스트 근거 없음)
...
CIRCLE_ENTRY_ENABLED = False           # 🔧 v3.3: 동그라미 비활성화 (새 로직으로 대체)
...
BOX_ENABLED = False                    # 🔧 v3.3: 박스권 비활성화 (새 로직으로 대체)
...
# 손절/모니터링
STOP_LOSS_PCT = 0.005  # 🔧 DYN_SL_MIN 0.5% 연동 (폴백용)
...
POSTCHECK_MAX_DD = 0.010  # 🔧 30일분석: HARD_STOP_DD 1.0% 연동
...
# =========================
# 🔧 v3.3 백테스트 기반 신호/필터 함수
# =========================

# 멀티TF 캔들 캐시 (60초 TTL)
_MTF_CACHE = {}  # {(unit, market): {"candles": [...], "ts": float}}
_MTF_CACHE_TTL = 30  # 30초 캐시 (스캔 루프 주기 고려)

def _get_candles_cached(unit, market, count=30):
    """멀티TF 캔들 캐시 조회 (API 호출 최소화)"""
    key = (unit, market)
    now = time.time()
    cached = _MTF_CACHE.get(key)
    if cached and (now - cached["ts"]) < _MTF_CACHE_TTL:
        return cached["candles"]
    candles = get_minutes_candles(unit, market, count)
    if candles:
        _MTF_CACHE[key] = {"candles": candles, "ts": now}
    return candles or []


def calc_ema_series(closes, period):
    """EMA 시리즈 계산 (전체 리스트 반환)"""
...
def check_15m_macd_golden(market):
    """15분봉 MACD 골든크로스 확인
    조건: MACD > Signal (골든크로스 상태) + MACD 양수 전환 중
    Returns: bool
    """
    c15 = _get_candles_cached(15, market, 40)
    if not c15 or len(c15) < 35:
        return False
    closes = [c["trade_price"] for c in c15]
    macd_line, signal_line, _ = calc_macd(closes)
    if macd_line is None or not macd_line or not signal_line:
        return False
    # 현재 MACD > Signal (골든크로스 상태)
    return macd_line[-1] > signal_line[-1]


def check_1h_ema_alignment(market):
    """1시간봉 EMA 정배열 확인
    조건: EMA5 > EMA10 > EMA20 (강세 정배열)
    Returns: bool
    """
    c60 = _get_candles_cached(60, market, 25)
    if not c60 or len(c60) < 21:
        return False
    ...
    if not ema5 or not ema10 or not ema20:
        return False
    return ema5[-1] > ema10[-1] > ema20[-1]


def check_upper_tf_strong(market):
    """상위 타임프레임 강세 필터 (Logic 1&2용)
    15m/30m/60m 조건 중 하나 이상 충족 시 통과:
    - 15m: VR20>2, 모멘텀10>3%
    - 30m: EMA정배열3+, VR5>3, 모멘텀10>3%
    - 60m: EMA정배열3+, VR5>3, 모멘텀10>3%, 위치20>80%
    ...
    if c15 and len(c15) >= 21:
        closes15 = [c["trade_price"] for c in c15]
        vols15 = [c.get("candle_acc_trade_volume", 0) for c in c15]
        # 15m VR20 > 2
        if len(vols15) >= 25:
            recent5 = sum(vols15[-5:])
            past20 = sum(vols15[-25:-5]) / 20.0 if sum(vols15[-25:-5]) > 0 else 1
            vr20 = recent5 / (5 * max(past20, 1e-10))
            if vr20 > 2:
                passed.append("15m_VR20>2")
        # 15m 모멘텀10 > 3%
        if len(closes15) >= 11:
            m10 = closes15[-1] / max(closes15[-11], 1) - 1.0
            if m10 > 0.03:
                passed.append(f"15m_모멘텀{m10*100:.1f}%")
    ...
    for tf in [30, 60]:
        ...
        # EMA정배열3+ (최근 3봉 연속 EMA5>EMA10>EMA20)
        ema5 = calc_ema_series(closes, 5)
        ema10 = calc_ema_series(closes, 10)
        ema20 = calc_ema_series(closes, 20)
        ...
        if len(volumes) >= 25:
            ...
            if vr5 > 3:
                passed.append(f"{tf}m_VR5={vr5:.1f}")

        # 모멘텀10 > 3%
        if len(closes) >= 11:
            m10 = closes[-1] / max(closes[-11], 1) - 1.0
            if m10 > 0.03:
                passed.append(f"{tf}m_모멘텀{m10*100:.1f}%")

        # 위치20 > 80% (60m만)
        if tf == 60 and len(candles) >= 20:
            highs = [c.get("high_price", c["trade_price"]) for c in candles[-20:]]
            lows = [c.get("low_price", c["trade_price"]) for c in candles[-20:]]
            h20 = max(highs)
            l20 = min(lows)
            if h20 > l20:
                pos20 = (closes[-1] - l20) / (h20 - l20)
                if pos20 > 0.80:
                    passed.append(f"60m_위치{pos20*100:.0f}%")

    if passed:
        return True, "+".join(passed[:3])
    return False, "상위TF조건없음"


def detect_20bar_high_breakout(c1):
    """20봉 고점돌파 신호 감지 (1분봉)
    조건: 현재가 > 과거 20봉의 최고가 (현재봉 제외)
    Returns: bool
    """
    if not c1 or len(c1) < 21:
        return False
    cur_price = c1[-1]["trade_price"]
    past_highs = [c.get("high_price", c["trade_price"]) for c in c1[-21:-1]]
    if not past_highs:
        return False
    return cur_price > max(past_highs)


def detect_3x_volume(c1):
    """거래량 3배 신호 감지 (1분봉)
    조건: 현재봉 거래대금 > 이전 20봉 평균의 3배
    Returns: bool, float (volume_ratio)
    """
    if not c1 or len(c1) < 21:
        return False, 0
    cur_vol = c1[-1].get("candle_acc_trade_price", 0)
    past_vols = [c.get("candle_acc_trade_price", 0) for c in c1[-21:-1] if c.get("candle_acc_trade_price", 0) > 0]
    if not past_vols or len(past_vols) < 5:
        return False, 0
...
def detect_15m_pullback_reversal(market):
    """15분봉 눌림반전 감지 (Logic 3용)
    조건: 최근 3봉 중 2봉+ 음봉 → 마지막 봉 양봉 (반전)
    Returns: str or None ("15m_눌림반전" or "15m_눌림+돌파" or None)
    """
    c15 = _get_candles_cached(15, market, 20)
    if not c15 or len(c15) < 5:
        return None
    last = c15[-1]
    if last["trade_price"] <= last["opening_price"]:
        return None  # 현재봉 양봉 아님
    # 이전 3봉 중 음봉 수
    bearish = sum(1 for c in c15[-4:-1] if c["trade_price"] <= c["opening_price"])
    if bearish < 2:
        return None
    # 이전봉 고가 돌파 여부
    prev_high = max(c.get("high_price", c["trade_price"]) for c in c15[-4:-1])
    if last["trade_price"] > prev_high:
        return "15m_눌림+돌파"
    return "15m_눌림반전"


def detect_5m_bullish_candle(market):
    """5분봉 양봉 감지 (Logic 4 서브트리거)
    조건: 마지막 5분봉이 양봉 + 몸통 비율 50%+ (실체가 큰 양봉)
    Returns: bool
    """
    c5 = _get_candles_cached(5, market, 5)
    if not c5 or len(c5) < 2:
        return False
    last = c5[-1]
    o, c = last["opening_price"], last["trade_price"]
    h = last.get("high_price", max(o, c))
    l = last.get("low_price", min(o, c))
    if c <= o:
        return False  # 음봉
    body = c - o
    total_range = h - l
...
def check_exclusion_filters(market, c1):
    ...
    # 3) EMA 역배열 (1분봉 EMA5 < EMA10 < EMA20)
    if len(closes) >= 20:
        ema5 = calc_ema_series(closes, 5)
        ema10 = calc_ema_series(closes, 10)
        ema20 = calc_ema_series(closes, 20)
...
# === 🔧 v3.3 백테스트 기반 진입 탐지 엔진 ===
def detect_leader_stock(m, obc, c1, tight_mode=False):
    """
    v3.3 백테스트 기반 4-로직 진입 탐지 엔진:
      Logic 1 (A): 20봉_고점돌파 + 상위TF강세 + TRAIL_SL1.0/A0.3/T0.2
      Logic 2 (A): 거래량3배 + 상위TF강세 + TRAIL_SL1.0/A0.3/T0.2
    ...
    if _coin_ticker in {"USDT", "USDC", "DAI", "TUSD", "BUSD"}:
    ...
    cur, prev = c1[-1], c1[-2]
    cur_price = cur["trade_price"]
    price_change = (cur_price / max(prev["trade_price"], 1) - 1)
