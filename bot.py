...
DYN_SL_MAX = 0.020   # 🔧 R:R개선: 3.5→2.0% (최대 SL도 축소, 1번 손절=10~17번이겨야→3~5번으로 개선)

# 🔧 통합 체크포인트: 트레일링/얇은수익/Plateau 발동 기준
# 🔧 구조개선: SL 연동 — 체크포인트 = SL × 1.5 (의미있는 수익에서만 트레일 무장)
#   기존 0.30%에서 무장 → 진입가+0.06%에 트레일스톱 → 한 틱에 트립 문제 해결
PROFIT_CHECKPOINT_BASE = 0.0035  # 0.60→0.35% (수수료0.1%+슬립0.1%+알파0.15% → 의미있는 수익에서만 Trail 무장)
...
MFE_RR_MULTIPLIERS = {
    "🔥점화": 2.5,              # 🔧 R:R개선: SL 1.2%×2.5=3.0% (점화는 크게 먹어야)
    "강돌파 (EMA↑+고점↑)": 2.0,  # 🔧 R:R개선: SL 1.2%×2.0=2.4%
    "EMA↑": 1.8,                 # 🔧 R:R개선: SL 1.2%×1.8=2.16%
    "고점↑": 1.8,                # 🔧 R:R개선: SL 1.2%×1.8=2.16%
    "거래량↑": 1.5,              # 🔧 R:R개선: SL 1.2%×1.5=1.8%
    "기본": 1.5,                 # 🔧 R:R개선: SL 1.2%×1.5=1.8%
}
...
TRAIL_DISTANCE_MIN_BASE = 0.0020  # 0.40→0.20% (SL 2.0%의 10% 수준, ATR 적응형 Trail로 보완)

def get_trail_distance_min():
    """🔧 R:R개선: 트레일 거리 확대
    기본 0.40% / 야간(0-7시) 0.30% (기존 0.10%는 너무 좁아 노이즈 트립)
    SL 1.2% × 0.25 = 0.30% (야간), SL 1.2% × 0.33 = 0.40% (주간)
    """
    _h = now_kst().hour
    if 0 <= _h < 7:
        return 0.0030  # 야간 0.30% (기존 0.10%는 너무 좁음)
    dyn_sl = DYN_SL_MIN
    return max(TRAIL_DISTANCE_MIN_BASE, dyn_sl * 0.25)  # SL 1.2% × 0.25 = 0.30% (BASE 0.40%가 우선)
...
def _apply_exit_profile():
    ...
    if prof == "gentle":
        ...
        TRAIL_ATR_MULT = 1.0
        TRAIL_DISTANCE_MIN_BASE = 0.0040  # 🔧 R:R개선: 0.15→0.40% (balanced 기본값과 통일)
        SPIKE_RECOVERY_WINDOW = 3
        SPIKE_RECOVERY_MIN_BUY = 0.58
        CTX_EXIT_THRESHOLD = 3
...
def get_adaptive_risk() -> float:
    ...
    if len(TRADE_HISTORY) >= 10:
        ...
        if win_rate < 0.30:
            base_risk = RISK_PER_TRADE * 0.8   # 🔧 R:R개선: 0.5→0.8 (과도한 축소→악순환 방지, 이겨도 미세이익 문제 해결)
        elif win_rate < 0.38:
            base_risk = RISK_PER_TRADE * 0.9   # 🔧 R:R개선: 0.7→0.9 (축소 바닥 제한으로 승률 회복 가능)
        elif win_rate >= 0.50:
...
GATE_GREEN_STREAK_MAX = 4     # 1010건분석: gs4+ 최적 (CP74% SL33%) → 5→4 (gs5+ 차단)
...
def is_afternoon_weak_period():
    """오후 약세 시간대 필터 (12:00-17:59 진입 차단)"""
    _h = now_kst().hour
    if _h == 12:
        return True   # 정오 전환 구간 차단
    if 13 <= _h < 18:
        return True   # 오후 약세 완전 차단
    return False
