...
# 🔧 WF데이터: SL 1.0% 기본 (TRAIL_SL1.0/A0.5/T0.3)
DYN_SL_MIN = 0.010   # 🔧 WF: SL 1.0%
DYN_SL_MAX = 0.015   # 🔧 WF: SL_MAX 1.5% (고변동 여유)

# 🔧 WF데이터: Activation 0.5% (TRAIL_SL1.0/A0.5/T0.3)
PROFIT_CHECKPOINT_BASE = 0.005  # 🔧 WF: Activation 0.5%
...
TRAIL_DISTANCE_MIN_BASE = 0.003  # 🔧 WF데이터: Trail 0.3% (TRAIL_SL1.0/A0.5/T0.3)
...
# 🔧 WF데이터: 피라미딩 비활성화 (WF에서 테스트되지 않은 로직)
USE_PYRAMIDING = False  # os.getenv("USE_PYRAMIDING", "1") == "1"
...
def monitor_position(m,
    ...
    try:
        while time.time() - start_ts <= horizon:  # 🔧 before1 복원 (MAX_RUNTIME→horizon)
            ...
            if _v4_max_bars > 0 and _elapsed_bars_15m >= _v4_max_bars and cur_gain > 0:
