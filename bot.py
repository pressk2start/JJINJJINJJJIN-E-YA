...
def _eval_a_bypass(features):
    checks = {
        ...
        "entry_spread_pct": features.get("entry_spread_pct", 999) <= 0.89,
    }
    score = sum(checks.values())
    if score == 4:
        return {"pass": True, "score": score, "size_pct": 0.50, "reason": "A_BYPASS_4OF4", "checks": checks}
    return {"pass": False, "score": score, "size_pct": 0.0, "reason": f"A_BYPASS_FAIL_{score}OF4", "checks": checks}
...
# 목적: raw N → gate 0 사이 어느 게이트가 지배적인가를 사유 분포로 관측 (log-only, 전략 무변경)
# 스테이지 분리 (raw88 → gate0 병목은 POST_SIGNAL 만이 답):
#   PRE_SIGNAL  = 신호 생성 전/미도달 (position, no_v4) — raw로 세지 않음
#   POST_SIGNAL = _v4_signal 존재 후 진입 완료 전 (10곳) — 여기가 raw→gate 병목
# debounce: (market, code) 10초 — market 단독이면 앞 조건이 뒤 조건을 가림
# TTL: 60초마다 stale(>120초) cleanup, 실제 실행 보장
_PRE_SIGNAL_FAIL_STATS = {}        # code -> total count
_POST_SIGNAL_GATE_FAIL_STATS = {}  # code -> total count
_PRE_SIGNAL_LAST_REPORT = {}       # delta 계산 snapshot
...
_DETECT_STAGE = {
    ...
    "detect_coin_cd":   "post_signal", # v4 pass 후 코인별 연패 쿨다운
    "detect_no_ticks":  "post_signal", # v4 pass 후 tick fetch 실패
    "detect_fake_flow": "post_signal", # 극단 스푸핑 하드컷
    "detect_tick_age":  "post_signal", # GT tick_age 부족
...
def _detect_gate_observe(market, code, **obs):
    ...
    """
    global _DETECT_GATE_LAST_CLEANUP
    try:
        now_ts = time.time()
        key = (market, code)
        with _GATE_FAIL_LOCK:
            if now_ts - _DETECT_GATE_SEEN.get(key, 0.0) < 10.0:
                return
            _DETECT_GATE_SEEN[key] = now_ts
            # 실제 실행되는 TTL cleanup (60초마다 stale >120s 제거)
            if now_ts - _DETECT_GATE_LAST_CLEANUP >= 60.0:
                cutoff = now_ts - 120.0
                for kk, ts in list(_DETECT_GATE_SEEN.items()):
                    if ts < cutoff:
                        _DETECT_GATE_SEEN.pop(kk, None)
                _DETECT_GATE_LAST_CLEANUP = now_ts
...
def _detect_stats_delta(stats_dict, last_report):
    """스테이지별 stats → {code: {count, delta}} 계산 + snapshot 갱신."""
    result = {}
    try:
        with _GATE_FAIL_LOCK:
            for code in sorted(stats_dict.keys()):
                total = stats_dict[code]
                last = last_report.get(code, 0)
                delta = total - last
                result[code] = {"count": total, "delta": delta}
                last_report[code] = total
    except Exception as e:
        print(f"[DETECT_SUMMARY_ERR] {e}")
    return result
...
def _detect_gate_format_summary(only_recent=True):
    ...
    for label, stats, last_report in [
        ("POST_SIGNAL_GATE_FAIL", _POST_SIGNAL_GATE_FAIL_STATS, _POST_SIGNAL_LAST_REPORT),
        ("PRE_SIGNAL_FAIL",      _PRE_SIGNAL_FAIL_STATS,      _PRE_SIGNAL_LAST_REPORT),
    ]:
        d = _detect_stats_delta(stats, last_report)
        if not d:
            continue
...
def _pipeline_report(force=False):
    ...
    try:
        ...
        pass
    # detect_leader 게이트 스테이지별 SUMMARY (v2): POST_SIGNAL을 먼저
    try:
        _det_summary = _detect_gate_format_summary(only_recent=True)
        if _det_summary:
    ...
    # ━━━━ Tier 2: PIPELINE (condensed) ━━━━
    lines.append(
        f"PIPE: scan{c['scan_markets']}"
        f" v4 {_v4}({pct(_v4, _det)})")
    lines.append(
        f"  raw{_raw} gate{_gp}"
    ...
    for _gk, _gl in [("gate_fail_no_v4","no_v4"), ("gate_fail_coin_cd","cd"),
        ...
        _gv = c.get(_gk, 0)
        if _gv > 0:
            _gate_raw.append((_gl, _gv))
    if _gate_raw:
        _gate_raw.sort(key=lambda x: -x[1])
        _gate_top = _gate_raw[:5]
        _gate_parts = [f"{lb}:{ct}" for lb, ct in _gate_top]
        _extra = len(_gate_raw) - 5
        _bl = f"BLOCK: {' '.join(_gate_parts)}"
        if _extra > 0:
            _bl += f" +{_extra}"
        lines.append(_bl)
    if c.get('suspend_block', 0) > 0:
    ...
    with _CHECK_FN_CACHE_LOCK:
        _total_hits = sum(_CHECK_FN_CACHE_HITS.values())
        _total_misses = sum(_CHECK_FN_CACHE_MISSES.values())
    _total_cache = _total_hits + _total_misses
    if _total_cache > 0:
        _cache_rate = _total_hits / _total_cache * 100
    # stage p95≥50ms 카운트
    _stage_short = {"scan_fetch":"sfetch", "scan_detect":"sdet",
                    "detect_leader":"dlead", "dl_c1_fetch":"c1",
                    "dl_multitf_fetch":"mtf", "shadow_eval":"eval"}
    _stage_rows = []
    with _PIPELINE_STAGE_LOCK:
        stage_snapshot = {k: list(v) for k, v in _PIPELINE_STAGE_LATENCIES.items()}
    for _sn in ("scan_fetch", "scan_detect", "detect_leader", "dl_c1_fetch",
                "dl_multitf_fetch", "shadow_eval"):
        _lst = stage_snapshot.get(_sn, [])
        if len(_lst) >= 5:
            _s_p95 = sorted(_lst)[min(int(len(_lst) * 0.95), len(_lst) - 1)]
            if _s_p95 >= 50:
                _stage_rows.append(f"{_stage_short.get(_sn,_sn)}:{_s_p95:.0f}ms")
    # shadow VP 추적 수
    with _SHADOW_LOCK:
        _shadow_active = len(_SHADOW_VIRTUAL_POSITIONS)
    # 1줄 debug 요약
    _dbg_parts = []
    if _scan_p95_s > 0:
    ...
    if _shadow_active > 0:
        _dbg_parts.append(f"shd{_shadow_active}")
    if c.get('suspend_block', 0) > 0:
    ...
    for _sname, _sconf in _STRATEGY_REGISTRY.items():
        ...
        if _fails:
            ...
            _fail_str = " ".join(f"{fn}:{fv}" for fn, fv in _fails[:8])
    ...
    _val_parts = []
    if _m3.get("n", 0) > 0:
        _val_parts.append(f"m3:NM{_m3['near_miss']}")
    if _vr5.get("n", 0) > 0:
        _val_parts.append(f"VR5:NM{_vr5['near_miss']}")
    if _atr.get("n", 0) > 0:
...
def _a_daily_guard_ok(market=None):
    ...
    if market:
        ...
        if same_coin:
            return False, f"A군 동일코인 {market} 당일 재진입 제한"
    return True, ""
...
_STRAT_DESC_MAP = {
    ...
    # 기각: CLM_H180(-0.01% MAE-0.46%), CLM_H120(0.00%) — exit 지연 무효 (n=55, 39차)
    # 기각: CLM_CS, CLM_BC — check_fn 중복
...
# === A군 bypass 전용 exit: GT식 240s hold + dd 0.5% gate (SVE1 120s → GT 240s 전환) ===
_V0_EXIT_PARAMS_A_BYPASS = {
    "strategy": "TRAIL",
    "sl_pct": 0.020,
    "activation_pct": 1.0,
    "trail_pct": 0.005,
    "hold_bars": 0,
    "max_bars": 80,            # 240s (80 × 3s)
    "disable_trail": True,
    "sl_tiers": [(60, 0.025), (120, 0.015), (9999, 0.010)],
    "survival_gate_sec": 60,
...
_V0_EXIT_PARAMS_LATE_CONT = {
    "strategy": "TRAIL",
    "sl_pct": 0.020,
    "activation_pct": 1.0,
    "trail_pct": 0.005,
    "hold_bars": 0,
    "max_bars": 100,            # 300s (100 × 3s) — FBR/CLMP 늦은 continuation 포착
    "disable_trail": True,
    "sl_tiers": [
        (60,   0.035),  # 0~60s: 3.5% (초반 DD 완전 허용 — CLMP 60s:-0.29%)
        (120,  0.025),  # 60~120s: 2.5% (중간 DD 허용)
        (180,  0.015),  # 120~180s: 1.5% (반전 구간 진입)
        (9999, 0.010),  # 180s+: 1.0% (확정 continuation 보호)
    ],
    "description": "LATE_CONT:max300s/wideSL(3.5/2.5/1.5/1.0)/no-trail",
}
...
def _make_momentum_vr5low_variant(rsi_min, vr5_max, pkey):
    """GT + vr5 상한 shadow variant — 자동 탐지 결과 활용
    근거: W=0.476 L=0.938 (d=0.82) → vr5가 낮을 때 승리 확률 높음
    vr5 ≤ 0.707 = 추천 상한. 낮은 vr5 = 매수파 초입 진입 (pile-in 전)"""
    def _check(c1, c5, c15, c30, c60, gate_info=None):
        _pipeline_inc(f"{pkey}_enter")
        if not c5 or len(c5) < 15:
            return None
        ...
        if not c1 or not _v4_is_bullish(c1[-1]):
            ...
            if _pipeline_inc(f"{pkey}_1m_fail", value=round(_bp, 2), threshold=0, direction="gt"): return None
        _vr5 = _v4_volume_ratio_5(c1) if c1 and len(c1) >= 7 else None
        # v18i-vr5: 상한 3.2에 더해 vr5_max(0.707) 추가 — 초입 진입 알파 검증
        if _vr5 is not None and _vr5 > 3.2:
            if _pipeline_inc(f"{pkey}_vr5_over_fail", value=round(_vr5, 2), threshold=3.2, direction="lte"): return None
        if _vr5 is None or _vr5 > vr5_max:
            if _pipeline_inc(f"{pkey}_vr5_high_fail", value=round(_vr5, 2) if _vr5 else None, threshold=vr5_max, direction="lte"): return None
        _vr15 = None
        if c15 and len(c15) >= 6:
            cur_vol = c15[-1].get("candle_acc_trade_price", 0)
            past = [c.get("candle_acc_trade_price", 0) for c in c15[-6:-1]]
        ...
        return {
            ...
            "entry_mode": "confirm",
            "logic_group": "G",
            "filters_hit": [f"5mRSI={rsi_5m:.1f}", f"VR5={_vr5}(<={vr5_max})", f"VR15={_vr15}"],
            "exit_params": _V0_EXIT_PARAMS_MOMENTUM.copy(),
            "indicators": {"rsi_5m": round(rsi_5m, 1)},
        }
    return _check
...
_v0_check_momentum_vr5low = _make_momentum_vr5low_variant(74.55, 0.707, "momentum_vr5low")
...
# === v21 신규 시나리오 진입 함수 (7개) ===

_COIN_BIAS = {}  # {market: (wins, losses, avg_pnl)} — SVE1 coin_wl에서 주기적 갱신
    ...
    with _SHADOW_PERF_LOCK:
        for key, s in _SHADOW_PERF_STATS.items():
            if not key.startswith("SVE1:"):
                continue
            cw = s.get("coin_wl", {})
            trs = s.get("trade_records", [])
            coin_pnls = {}
            for tr in trs:
                m = tr.get("market", "")
                if m:
                    coin_pnls.setdefault(m, []).append(tr.get("pnl", 0))
            for coin, (w, l) in cw.items():
                avg_pnl = 0
                if coin in coin_pnls and coin_pnls[coin]:
                    avg_pnl = sum(coin_pnls[coin]) / len(coin_pnls[coin])
                _COIN_BIAS[coin] = (w, l, avg_pnl)
            break
...
def _v0_check_quiet_cont(c1, c5=None, c15=None, c30=None, c60=None, gate_info=None):
    ...
    if hist_5m is None or hist_5m <= 0:
        if _pipeline_inc("quiet_cont_macd_neg"): return None
    price_5m = max(closes_5m[-1], 1)
    macd_bps = hist_5m / price_5m * 10000
    if macd_bps > 80:
        if _pipeline_inc("quiet_cont_macd_hot", value=round(macd_bps, 1), threshold=80, direction="lte"): return None
    vr_15m = None
    if c15 and len(c15) >= 6:
        cur_v15 = c15[-1].get("candle_acc_trade_price", 0)
        past_v15 = [c.get("candle_acc_trade_price", 0) for c in c15[-6:-1]]
    ...
    if gap_pct > 0.8:
        if _pipeline_inc("quiet_cont_gap_fail", value=round(gap_pct, 2), threshold=0.8, direction="lte"): return None
    _pipeline_inc("quiet_cont_pass")
    return {
        "signal_tag": "조용한continuation",
        "entry_mode": "confirm",
        "logic_group": "QC",
        "filters_hit": [f"RSI5={rsi_5m:.1f}", f"ATR={atr:.3f}", f"MACD={macd_bps:.1f}bps", f"VR15={vr_15m}", f"gap20={gap_pct:.2f}%"],
        "exit_params": {},
        "indicators": {"rsi_5m": round(rsi_5m, 1), "atr_pct": round(atr, 4), "macd_bps": round(macd_bps, 1), "vr_15m": vr_15m},
    }


def _v0_check_shakeout_reclaim(c1, c5=None, c15=None, c30=None, c60=None, gate_info=None):
    """SHR shakeout reclaim: -0.3~-0.8% 급락 후 EMA20 복귀 + tick_buy + RSI50-68"""
    ...
    recent_high = max(c["high_price"] for c in c1[-6:-1])
    ...
    prev_low = min(c["low_price"] for c in c1[-8:-5]) if len(c1) >= 8 else recent_low
    if recent_low < prev_low * 0.997:
        if _pipeline_inc("shakeout_reclaim_newlow_fail"): return None
    rsi_5m = _v4_rsi_from_candles(c5, 14)
    if rsi_5m is None or rsi_5m < 50 or rsi_5m > 68:
        if _pipeline_inc("shakeout_reclaim_rsi_fail", value=rsi_5m, threshold=50): return None
    _pipeline_inc("shakeout_reclaim_pass")
    return {
        "signal_tag": "shakeout복귀",
        "entry_mode": "confirm",
        "logic_group": "SHR",
        "filters_hit": [f"dip={dip_pct:.2f}%", f"RSI5={rsi_5m:.1f}", f"EMA20={ema20:.0f}"],
        "exit_params": {},
        "indicators": {"dip_pct": round(dip_pct, 3), "rsi_5m": round(rsi_5m, 1)},
    }
...
def _v0_check_clm_pullback(c1, c5=None, c15=None, c30=None, c60=None, gate_info=None, _depth_hi=-0.15, _depth_lo=-1.0):
    ...
    pullback_candles = c1[len(c1) + climax_idx + 1:-1] if climax_idx < -1 else []
    if not pullback_candles:
        if _pipeline_inc("clm_pullback_too_short"): return None
    climax_close = c1[climax_idx]["trade_price"]
    ...
    if c1[-1]["trade_price"] < midpoint:
        if _pipeline_inc("clm_pullback_hold_fail"): return None
    _pipeline_inc("clm_pullback_pass")
    return {
        "signal_tag": "CLM눌림",
        "entry_mode": "confirm",
        "logic_group": "CLMP",
        "filters_hit": [f"pb={pullback_pct:.2f}%", f"gap={abs(climax_idx)}봉전"],
        "exit_params": {},
...
def _v0_check_range_expand(c1, c5=None, c15=None, c30=None, c60=None, gate_info=None):
    """RX range expansion: 20봉고저폭≤70% + ATR<0.8 + 20봉돌파 + 거래대금↑1.5x"""
    _pipeline_inc("range_expand_enter")
    if not c1 or len(c1) < 25 or not _v4_is_bullish(c1[-1]):
        if _pipeline_inc("range_expand_bull_fail"): return None
    recent_20 = c1[-21:-1]
    r20_high = max(c["high_price"] for c in recent_20)
    r20_low = min(c["low_price"] for c in recent_20)
    r20_range = (r20_high - r20_low) / max(r20_low, 1) * 100
    older_20 = c1[-25:-5] if len(c1) >= 25 else c1[:20]
    o20_high = max(c["high_price"] for c in older_20) if older_20 else r20_high
    o20_low = min(c["low_price"] for c in older_20) if older_20 else r20_low
    o20_range = (o20_high - o20_low) / max(o20_low, 1) * 100
    if o20_range <= 0 or r20_range / o20_range > 0.70:
        if _pipeline_inc("range_expand_compress_fail"): return None
    cur_atr = _v4_atr_pct(c1, 14)
    if cur_atr is None or cur_atr <= 0:
        return None
    old_candles = c1[:-7]
    ...
    atr_ratio = cur_atr / old_atr
    if atr_ratio >= 0.8:
        if _pipeline_inc("range_expand_atr_fail", value=round(atr_ratio, 2), threshold=0.8, direction="lt"): return None
    cur_close = c1[-1]["trade_price"]
    if cur_close <= r20_high:
        if _pipeline_inc("range_expand_breakout_fail"): return None
    cur_vol = c1[-1].get("candle_acc_trade_price", 0)
    prev3_vol = sum(c.get("candle_acc_trade_price", 0) for c in c1[-4:-1]) / 3
    if prev3_vol <= 0 or cur_vol / prev3_vol < 1.5:
        if _pipeline_inc("range_expand_vol_fail"): return None
    hi, lo = c1[-1]["high_price"], c1[-1]["low_price"]
    rng = hi - lo
    if rng > 0:
        upper_wick = hi - max(cur_close, c1[-1]["opening_price"])
        if upper_wick / rng > 0.35:
            if _pipeline_inc("range_expand_wick_fail"): return None
    _pipeline_inc("range_expand_pass")
    return {
        "signal_tag": "범위확장",
        "entry_mode": "confirm",
        "logic_group": "RX",
        "filters_hit": [f"ATR_r={atr_ratio:.2f}", f"VR3={cur_vol/prev3_vol:.1f}", f"r20_ratio={r20_range/o20_range:.2f}"],
        "exit_params": {},
        "indicators": {"atr_ratio": round(atr_ratio, 3), "range_ratio": round(r20_range / max(o20_range, 0.001), 3)},
    }


def _v0_check_liquidity_trap(c1, c5=None, c15=None, c30=None, c60=None, gate_info=None):
    """LTRP 유동성함정: 5조건 중 3개 이상 → 함정 탐지 (차단용 역시나리오)"""
    ...
    rng = hi - lo
    spread_pct = rng / price * 100
    if spread_pct >= 1.0:
        score += 1
        details.append(f"spread={spread_pct:.2f}%")
    atr = _v4_atr_pct(c1, 14)
    if atr >= 1.1:
        score += 1
        details.append(f"ATR={atr:.3f}")
    if rng > 0:
        upper_wick = hi - max(c1[-1]["trade_price"], c1[-1]["opening_price"])
        if upper_wick / rng >= 0.35:
            score += 1
            details.append(f"wick={upper_wick/rng:.2f}")
    if c5 and len(c5) >= 35:
        closes_5m = [c["trade_price"] for c in c5]
        _, _, hist_5m = _v4_macd(closes_5m)
        if hist_5m is not None:
            macd_bps = hist_5m / max(closes_5m[-1], 1) * 10000
            if macd_bps >= 80:
                score += 1
                details.append(f"MACD={macd_bps:.0f}bps")
    vr5 = _v4_volume_ratio_5(c1)
    if vr5 >= 3.5:
        score += 1
        details.append(f"VR5={vr5:.1f}")
    if score < 3:
        if _pipeline_inc("liq_trap_score_fail", value=score, threshold=3, direction="gte"): return None
    ...
    return {
        ...
        "filters_hit": details,
        "exit_params": {},
        "indicators": {"trap_score": score, "spread_pct": round(spread_pct, 3), "vr5": round(vr5, 2)},
    }
...
def _v0_check_coin_personality(c1, c5=None, c15=None, c30=None, c60=None, gate_info=None):
    ...
    if not market or market not in _COIN_BIAS:
        if _pipeline_inc("coin_pers_no_data"): return None
    w, l, avg_pnl = _COIN_BIAS[market]
    total = w + l
    if total < 3:
    ...
    return {
        ...
        "entry_mode": "confirm",
        "logic_group": "CPRS",
        "filters_hit": [f"coin={coin_tag}", f"WR={coin_wr:.0f}%", f"PnL={avg_pnl*100:+.2f}%"],
        "exit_params": {},
        "indicators": {"coin_wr": round(coin_wr, 1), "avg_pnl": round(avg_pnl * 100, 2), "vr5": round(vr5, 2)},
    }


def _v0_check_failed_breakout(c1, c5=None, c15=None, c30=None, c60=None, gate_info=None):
    """FBR 실패돌파2차: 1차 돌파→밀림→저점높아짐→재돌파 (spread 감소 시 quality↑)"""
    _pipeline_inc("fbr_enter")
    if not c1 or len(c1) < 25 or not _v4_is_bullish(c1[-1]):
        if _pipeline_inc("fbr_bull_fail"): return None
    high_20 = max(c["high_price"] for c in c1[-25:-5])
    first_break = None
    for i in range(-10, -3):
        if abs(i) > len(c1):
            continue
        if c1[i]["high_price"] > high_20:
            first_break = i
            break
    if first_break is None:
        if _pipeline_inc("fbr_no_first_break"): return None
    post_break = c1[len(c1) + first_break + 1:-1]
    if not post_break or len(post_break) < 2:
        if _pipeline_inc("fbr_no_pullback"): return None
    pullback_close = min(c["trade_price"] for c in post_break)
    if pullback_close >= high_20:
        if _pipeline_inc("fbr_no_rejection"): return None
    pullback_low = min(c["low_price"] for c in post_break)
    pre_break_low = min(c["low_price"] for c in c1[-25:len(c1)+first_break])
    if pullback_low < pre_break_low:
        if _pipeline_inc("fbr_lower_low"): return None
    cur_close = c1[-1]["trade_price"]
    if cur_close <= high_20:
        if _pipeline_inc("fbr_no_rebreak"): return None
    first_spread = (c1[first_break]["high_price"] - c1[first_break]["low_price"]) / max(c1[first_break]["trade_price"], 1) * 100
    cur_spread = (c1[-1]["high_price"] - c1[-1]["low_price"]) / max(cur_close, 1) * 100
    spread_shrink = cur_spread < first_spread
    _pipeline_inc("fbr_pass")
    return {
        "signal_tag": "실패돌파2차",
        "entry_mode": "confirm",
        "logic_group": "FBR",
        "filters_hit": [f"1st={abs(first_break)}봉전", f"pb_low>pre_low", f"spread_shrink={spread_shrink}"],
        "exit_params": {},
        "indicators": {"first_break_gap": abs(first_break), "spread_shrink": 1 if spread_shrink else 0},
    }
...
# --- v0 전략 레지스트리 (v23: QA/QC/TAC180 정리, FBR/CLMP→LATE_CONT exit — Production 1 + Research 14) ---
# v22→v23: -QA(에너지부족) -QC(MFE약) -TAC180(실험실패) / FBR+CLMP→LATE_CONT(300s/wideSL)
_STRATEGY_REGISTRY = {
    ...
    # CLM_B60R5(-0.09%), CLM_B60AT(-0.06%): 조합 필터 전멸
    # CLM_H180(-0.01% MAE-0.46%), CLM_H120(0.00% MAE-0.36%): exit 지연 무효 (n=55)
    "과열감지_OBSLIP": {
        "check_fn": _v0_check_climax,
        "exit_params": _V0_EXIT_PARAMS_CLM_ADAPTIVE,
        "priority": 10, "enabled": False,
    ...
    # SVE1_GATE30 폐기: n=23, cap=-19%, PnL=-0.05%. gate 단독으론 SVE1 개선 실패
    # GT_GATE30 폐기: n=23, cap=-3%, PnL=-0.01%. 음수 전환
    ...
    with _SHADOW_PERF_LOCK:
        ...
        for key, s in sorted_stats:
            ...
            if route in _seen_routes:
                continue
            _seen_routes.add(route)
            if route not in _PRODUCTION_ROUTES and route not in _ACTIVE_RESEARCH:
                continue
            wins = s.get("wins", 0)
            wr = wins / n * 100
            avg_pnl = s.get("total_pnl", 0) / n * 100
            ...
            if route in _PRODUCTION_ROUTES:
                ...
                if route == "CLM" and _trs and len(_trs) >= 50:
                    ...
                    if len(_a_trs) >= 30:
                        ...
                        _comp_keys = ["rsi_5m", "rsi_15m", "atr_pct", "entry_spread_pct", "body_pct", "wick_ratio", "ema_spread_15", "tick_rate_30s", "tick_buy_30s", "tick_strength_30s", "ob_slip_sell_10000k", "close_strength", "adx_60"]
                        _comp_parts = []
    ...
    with _SHADOW_PERF_LOCK:
        if _SHADOW_BLOCKED_STATS:
            ...
            if _blocked_pending > 0:
                _blk_summary += f" ⏳수집{_blocked_pending}개"
            lines.append(_blk_summary)
            for _rl in _review_lines:
                lines.append(_rl)
    with _SHADOW_LOCK:
...
def detect_leader_stock(m, obc, c1=None, tight_mode=False):
    ...
    with _POSITION_LOCK:
        ...
        if pos:
            ...
            _detect_gate_observe(m, "detect_position")  # PRE_SIGNAL
            return None
    ...
    # v18h-tune3: 과필터 롤백 — raw_hit 0 재발 방지
    #   scan_detect 23s로 속도는 완성, 기회 확보 우선
    #   - spread: 0.6% → 0.65% (v18h-tune2 롤백: 158건 컷은 과함)
    #   - sell:   0.70 → 0.72 (v18h-tune2 롤백: 162건 컷으로 GT 초입 차단)
    #   - depth:  15M 유지 (41건, 의미있는 필터로 정착)
    #   - dead:   10M 유지 (255건, 주 필터)

    # 1) 스프레드 컷 (0.65% 초과 시 skip) — v18h-tune3: 0.6%에서 롤백
    if ob.get("spread", 0) > 0.65:
        _pipeline_inc("pre_cut_spread")
        return None

    # 2) 오더북 깊이 컷 (top-3 총액 15M KRW 미만 = 유동성 부족) — 유지
    if ob.get("depth_krw", 0) < 15_000_000:
        _pipeline_inc("pre_cut_depth")
        return None

    # 3) 매도 우세 컷 (top-3 호가창 ask/total > 0.72)
    # v18h-tune3: 0.70 → 0.72 (GT는 초기 매도벽 상태에서 시작되므로 완화)
    ...
    if _pre_units:
        ...
        if _pre_total > 0 and (_pre_askv / _pre_total) > 0.72:
    ...
    if not _v4_signal:
        ...
        _detect_gate_observe(m, "detect_no_v4")  # PRE_SIGNAL (raw 이전)
        return None
    ...
    if is_coin_loss_cooldown(m):
        ...
        _detect_gate_observe(m, "detect_coin_cd")  # 실측값 없음 → threshold 생략
        return None
    ...
    if twin["buy_ratio"] >= 0.98 and pstd10 is not None and pstd10 <= 0.001 and cv is not None and cv >= 2.5:
        ...
        _pipeline_inc("post_signal_blocked")
        # 복합 조건: 세 피연산자·threshold 모두 관측 (관측값·설정값 구분)
    ...
    if _gate_buy_ratio < GATE_BUY_RATIO_MIN:
        ...
        _pipeline_inc("post_signal_blocked")
        _detect_gate_observe(m, "detect_buy_ratio",
            buy_ratio=round(_gate_buy_ratio, 2), buy_ratio_thr=GATE_BUY_RATIO_MIN)
        return None
    ...
    _pipeline_inc("post_signal_pass")
            ...
            for m in shard:
              ...
              try:  # 🔧 심볼별 예외 격리 (한 심볼 에러가 전체 스캔 중단 방지)
                ...
                if _suspend_ts > time.time():
                    ...
                    if _bp_eval["pass"]:
                        _a_dg_ok, _a_dg_reason = _a_daily_guard_ok(m)
                        if not _a_dg_ok:
                    ...
                    if _bp_eval["pass"]:
                        ...
                        pre["v4_exit_params"] = _V0_EXIT_PARAMS_A_BYPASS.copy()
                        print(f"[A_BYPASS_DG] {m} A군 {_bp_eval['score']}/4 → daily_guard 우회 ({_bp_eval['size_pct']:.0%}) "
                              f"tr30={_bp_features.get('tick_rate_30s')} atr={_bp_features.get('atr_pct')} macd={_bp_features.get('macd_hist_5m_bps')} sprd={_bp_features.get('entry_spread_pct')} [GT_EXIT:240s]")
                        _pipeline_inc("suspend_a_bypass")
                        with _A_BYPASS_LOCK:
                            _A_BYPASS_PENDING[m] = _bp_row
                        ...
                        _append_a_bypass_sample(_bp_row)
                        cut("DAILY_GUARD", f"{m} {_daily_reason} (A불통과 {_bp_eval['score']}/4)")
                        _pipeline_inc("suspend_block")
                        _pipeline_inc("block_daily_guard")
                        continue
