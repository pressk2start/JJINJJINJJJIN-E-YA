...
### 3.7 [LOW] 점화 감지 window의 고정 시간 (10초)
...
### 3.8 [LOW] `rnd = _jitter` alias (L27)

**현상**: `_jitter` 함수의 alias `rnd`가 정의되어 있으나 사용처 불명
**권장**: 사용되지 않으면 제거

### 3.9 [LOW] `import traceback` 인라인 중복
...
### 개선 필요
- **파일 분리**: 12,000줄 단일 파일은 유지보수 한계 근접
- **전역 상태 캡슐화**: 20+ 전역 변수를 클래스로 관리
- **Lock 순서 명시**: 데드락 방지를 위한 문서화/강제
- **중복 계산 제거**: tick 분석 결과 캐싱
- **테스트 인프라**: 현재 단위테스트 없음 → 회귀 방지 어려움
